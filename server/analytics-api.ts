...
export const analyticsMiddleware = (
  ...
  res.on("finish", () => {
    ...
    console.log(
      `[Analytics] ${req.method} ${req.path} - ${res.statusCode} - ${responseTime}ms`,
    );
  });
...
export const collectPersonaEvents = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Analytics] Successfully saved ${events.length} events for session ${sessionId}`,
    );
...
export const updateCampaignPerformance = async (
  ...
  try {
    ...
    console.log(
      `[Analytics] Updated campaign performance for ${campaignId} - ${persona}`,
    );
...
export const generateAnalyticsReport = async (req: Request, res: Response) => {
  try {
    ...
    if (format === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="analytics-report-${timeRange}.csv"`,
      );
  ...
  for (const event of campaignEvents) {
    try {
      ...
      if (Object.keys(updateData).length > 0) {
        ...
        console.log(
          `[Analytics] Would update campaign ${campaignId} with:`,
          updateData,
        );
      }
    } catch (error) {
      console.error(
        "[Analytics] Error updating campaign metrics for event:",
        error,
      );
    }
  }
...
function calculateSummaryStats(events: any[]) {
  ...
  const totalConversions = events.filter(
    (e) => e.conversion_step === "conversion",
  ).length;
  const avgConfidence =
    events.length > 0
  ...
  const personaBreakdown = events.reduce(
    (acc, event) => {
...
function calculatePersonaMetrics(events: any[]) {
  ...
  return personas.map((persona) => {
    ...
    const conversions = personaEvents.filter(
      (e) => e.conversion_step === "conversion",
    );
    const revenue = conversions.reduce(
      (sum, e) => sum + (e.metadata?.revenue || 0),
      0,
    );
    ...
    return {
      ...
      conversionRate:
        personaEvents.length > 0
          ? (conversions.length / personaEvents.length) * 100
          : 0,
      revenue,
      avgConfidence:
        personaEvents.length > 0
          ? (personaEvents.reduce((sum, e) => sum + (e.confidence || 0), 0) /
...
function calculateConversionFunnel(events: any[]) {
  ...
  return funnelSteps.map((step) => ({
    ...
    percentage:
      events.length > 0
        ? (events.filter((e) => e.conversion_step === step).length /
            events.length) *
          100
        : 0,
  }));
...
function getTopPerformingCampaigns(campaigns: any[]) {
  return campaigns
    ...
    .map((campaign) => ({
      ...
      conversionRate: (campaign.conversion_rate * 100).toFixed(2) + "%",
      roas:
        campaign.revenue > 0 && campaign.cost_per_acquisition > 0
          ? (
              campaign.revenue /
              (campaign.conversions * campaign.cost_per_acquisition)
            ).toFixed(2)
          : "N/A",
    }));
}
...
function generateRecommendations(analyticsData: any[], campaignData: any[]) {
  ...
  const lowPerformingPersonas = personaMetrics.filter(
    (p) => p.conversionRate < 10,
  );
  ...
  const lowPerformingCampaigns = campaignData.filter(
    (c) => c.conversion_rate < 0.05,
  );
