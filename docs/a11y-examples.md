...
This guide provides real-world examples showing how to implement accessible components and fix common accessibility issues in React/TypeScript applications.
  ...
  return (
    ...
    >
      <span className="sr-only">
        {alarm.enabled ? 'Disable' : 'Enable'} alarm
      </span>
      <span
        className={cn(
...
function BadButton() {
  return (
    <button onClick={doSomething} className="text-gray-400"> {/* Low contrast */}
...
function BadCustomButton() {
  return (
    <div onClick={handleClick} className="looks-like-button"> {/* Not keyboard accessible */}
      Click me
    </div>
  );
}
...
function BadFieldset() {
  return (
    <div className="group"> {/* Should use fieldset */}
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
          ...
          <p id="dialog-description" className="text-gray-600 mb-6">
            Are you sure you want to delete "{alarm.name}"? This alarm will be permanently 
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify your alarm settings below.
          </DialogDescription>
        </DialogHeader>
...
describe('AlarmToggleButton Accessibility', () => {
  ...
  it('should be accessible when enabled', async () => {
    await axeRender(
      <AlarmToggleButton alarm={mockAlarm} onToggle={jest.fn()} />
    );
  });
  ...
  it('should be keyboard accessible', async () => {
    ...
    const { container } = render(
      <AlarmToggleButton alarm={mockAlarm} onToggle={onToggle} />
    );
...
This examples file provides real-world, copy-paste-ready code that developers can use to implement accessible components in the Relife alarm app. Each example shows both the correct implementation and common mistakes to avoid.