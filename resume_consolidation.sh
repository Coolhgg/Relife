...
# Use existing timestamp from the running consolidation
EXISTING_LOG_DIR="ci/step-outputs/consolidation-20250827_104923"
LOG_DIR="${EXISTING_LOG_DIR}"

echo "=== RESUMING CONSOLIDATION ==="
echo "Target branch: ${TARGET_BRANCH}"
echo "Keeping apps: ${MAIN_APP_DIR}, ${EMAIL_APP_DIR}"
echo "Using existing logs: ${LOG_DIR}"

# Set git merge driver for better conflict resolution
git config merge.ours.driver true

# Read the existing branches list and successful merges
mapfile -t ALL_BRANCHES < <(tail -n +2 "${LOG_DIR}/branches_list.txt")
if [[ -f "${LOG_DIR}/successful_merges.txt" ]]; then
    mapfile -t MERGED_BRANCHES < "${LOG_DIR}/successful_merges.txt"
else
    MERGED_BRANCHES=()
fi

echo "Total branches to process: ${#ALL_BRANCHES[@]}"
echo "Already merged: ${#MERGED_BRANCHES[@]}"
...
echo "RESUMING branch merging process..." | tee -a "${MERGE_LOG}"
...
for branch in "${REMAINING_BRANCHES[@]}"; do
  ...
  if (( (merged_count + failed_count) % 10 == 0 )); then
    echo "Progress: Processed $((merged_count + failed_count)) branches (${merged_count} merged, ${failed_count} failed/skipped)" | tee -a "${MERGE_LOG}"
  fi
done

echo "====" | tee -a "${MERGE_LOG}"
echo "Final Merge Summary:" | tee -a "${MERGE_LOG}"
...
# Now continue with app organization and PR creation
TIMESTAMP="20250827_104923"  # Use the same timestamp
...
# 5) Get the current consolidation branch name
CONSOL_BRANCH=$(git branch --show-current)

# 6) Push consolidated branch to remote
echo "====" | tee -a "${MERGE_LOG}"
echo "Pushing consolidated branch to remote..." | tee -a "${MERGE_LOG}"
git push "${REMOTE}" "${CONSOL_BRANCH}" -u || echo "Branch already pushed or push failed"

# 7) Create a PR using gh if available
...
- Moved to backup: $(cat "${LOG_DIR}/apps_moved.txt" | grep "Moving" | wc -l 2>/dev/null || echo "0") directories

Please review conflicts in \`${CONFLICT_SUMMARY}\` and test functionality before merging."

  gh pr create --base "${TARGET_BRANCH}" --head "${CONSOL_BRANCH}" --title "Consolidation: merge feature branches into ${TARGET_BRANCH}" --body "${PR_BODY}" 2>/dev/null || echo "PR creation failed or PR already exists"
  echo "Created/Updated PR for ${CONSOL_BRANCH} -> ${TARGET_BRANCH}" | tee -a "${MERGE_LOG}"
else
