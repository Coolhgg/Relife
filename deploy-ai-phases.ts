  ...
  async deployAllPhases(): Promise<boolean> {
    ...
    try {
      ...
      await this.alerting.sendAlert(
        'critical',
        'Deployment Failed',
        `Critical error during deployment: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'deployment-system'
      );
      return false;
    }
  ...
  private async handleDeploymentFailure(failedPhase: number): Promise<void> {
    ...
    try {
      ...
      await this.alerting.sendAlert(
        ...
        `Phase ${failedPhase} deployment failed and was rolled back`,
        'deployment-system'
      );
    } catch (rollbackError) {
      ...
      await this.alerting.sendAlert(
        'critical',
        'Rollback Failed',
        `Failed to rollback Phase ${failedPhase}: ${rollbackError instanceof Error ? rollbackError.message : 'Unknown error'}`,
        'deployment-system'
      );
    }
  }
  ...
  private async validateDeployment(): Promise<boolean> {
    try {
      ...
      if (allPhasesCompleted) {
        ...
        console.log(
          `📊 Service Health: ${healthyServices}/${health.length} services healthy`
        );
        ...
        if (healthyServices === health.length) {
          ...
          console.warn(
            `⚠️  ${health.length - healthyServices} services are not healthy`
          );
          return false;
        }
      } else {
          ...
          phase.status === 'completed'
            ...
            : phase.status === 'in_progress'
              ...
              : phase.status === 'failed'
                ...
                : '⏳';
        console.log(
          `  ${statusIcon} Phase ${phase.phase}: ${phase.status} (${phase.progress}%)`
        );
      }
        ...
        const healthIcon =
          service.status === 'healthy'
            ...
            : service.status === 'degraded'
              ...
              : '❌';
        console.log(
          `  ${healthIcon} ${service.serviceName}: ${service.status} (${service.responseTime}ms)`
        );
      }
