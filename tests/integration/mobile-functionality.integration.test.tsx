...
import { integrationTestHelpers, simulateScreenWakeLock } from '../utils/integration-test-setup';
...
describe('Mobile-Specific Functionality Integration', () => {
  ...
  describe('Mobile Device Capabilities', () => {
    ...
    it('should handle mobile notification permissions and setup', async () => {
      ...
      const notificationSetup = screen.getByRole('button', { name: /setup.*notifications/i });
      ...
      await waitFor(() => {
        expect(screen.getByText(/notifications.*enabled|ready.*alarms/i)).toBeInTheDocument();
      });
    });
  });
