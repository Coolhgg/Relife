...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.gamingButton = page
      .getByRole("button")
      .filter({ hasText: /gaming|battle/i });
    ...
    this.upcomingAlarmsSection = page.locator(
      '[data-testid="upcoming-alarms"]',
    );
  ...
  async verifyRecentAlarms() {
    if (await this.recentAlarmsSection.isVisible()) {
      const recentAlarms = this.page.locator(
        '[data-testid="recent-alarm-item"]',
      );
  ...
  async verifyUpcomingAlarms() {
    if (await this.upcomingAlarmsSection.isVisible()) {
      const upcomingAlarms = this.page.locator(
        '[data-testid="upcoming-alarm-item"]',
      );
