...
let testAlarms: Map<string, Alarm> = new Map();
let testSubscriptions: Map<string, any> = new Map();
...
const generateSubscriptionId = () => `sub_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
...
export const alarmHandlers = [
  ...
  http.get(`${SUPABASE_URL}/rest/v1/alarms`, ({ request }) => {
    ...
    alarms.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  http.post(`${SUPABASE_URL}/rest/v1/alarms`, async ({ request }) => {
    ...
    if (user?.subscriptionTier === 'free') {
      const userAlarms = Array.from(testAlarms.values())
        .filter(alarm => alarm.userId === alarmData.userId);
      
      if (userAlarms.length >= 5) {
        return HttpResponse.json(
          { error: 'Free users can only have 5 alarms. Upgrade to premium for unlimited alarms.' },
          { status: 403 }
        );
      }
    }
    ...
    const alarm: Alarm = {
      ...
      dayNames: alarmData.dayNames || ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'],
  ...
  http.post(`${SUPABASE_URL}/rest/v1/alarms/:id/trigger`, async ({ params, request }) => {
  ...
  // Dismiss alarm
  http.post(`${SUPABASE_URL}/rest/v1/alarms/:id/dismiss`, async ({ params, request }) => {
    const alarmId = params.id as string;
    const body = await request.json() as any;
...
export const premiumHandlers = [
  ...
  http.get(`${STRIPE_URL}/v1/subscriptions`, ({ request }) => {
    ...
    const subscriptions = Array.from(testSubscriptions.values())
      .filter(sub => !customerId || sub.customer === customerId);
  ...
  http.post(`${STRIPE_URL}/v1/subscriptions`, async ({ request }) => {
    ...
    const subscription = {
      ...
      trial_end: body.trial_period_days ? Math.floor(Date.now() / 1000) + (body.trial_period_days * 24 * 60 * 60) : null
    };
    ...
    if (body.metadata?.userId) {
      ...
      if (user) {
        ...
        user.premiumFeatures = ['nuclear_mode', 'custom_sounds', 'unlimited_alarms', 'smart_wakeup'];
  ...
  http.get(`${SUPABASE_URL}/rest/v1/feature-access/:userId`, ({ params }) => {
    ...
    const hasActivePremium = user.subscriptionTier === 'premium' || user.subscriptionTier === 'trial';
    ...
    return HttpResponse.json({
      ...
      usage: {
        currentAlarms: Array.from(testAlarms.values()).filter(a => a.userId === userId).length
      }
    });
  })
];
...
export const voiceHandlers = [
  ...
  http.post(`${VOICE_API_URL}/recognize`, async ({ request }) => {
    ...
    const mockResults = [
      ...
      { transcript: 'create alarm', confidence: 0.90 },
  ...
  http.post('/api/voice/validate-command', async ({ request }) => {
    ...
    const validCommands = [
      'stop', 'stop alarm', 'dismiss', 'dismiss alarm', 'turn off',
      'snooze', 'snooze alarm', 'sleep', 'five more minutes',
      'create alarm', 'new alarm', 'add alarm',
      'settings', 'open settings', 'profile'
    ];
    
    const isValid = validCommands.some(valid => 
      command.includes(valid) || valid.includes(command)
    );
    
    let action = 'unknown';
    if (command.includes('stop') || command.includes('dismiss') || command.includes('turn off')) {
      action = 'dismiss_alarm';
    } else if (command.includes('snooze') || command.includes('sleep')) {
      action = 'snooze_alarm';
    } else if (command.includes('create') || command.includes('new') || command.includes('add')) {
      action = 'create_alarm';
