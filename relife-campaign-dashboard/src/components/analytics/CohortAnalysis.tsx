...
export function CohortAnalysis({ className }: CohortAnalysisProps) {
  ...
  const periods = ['Week 0', 'Week 1', 'Week 2', 'Week 3', 'Week 4', 'Week 8', 'Week 12'];
  const periodKeys: (keyof CohortData)[] = ['week0', 'week1', 'week2', 'week3', 'week4', 'week8', 'week12'];
            ...
            <thead>
              <tr className="border-b">
                ...
                {periods.map((period, index) => (
                  <th key={period} className="text-center p-2 font-medium">{period}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                    ...
                    <td key={key} className="p-2 text-center">
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>{cohort[key].toFixed(1)}% retention in {periods[index]}</p>
                            <p>{Math.round((cohort[key] / 100) * cohort.size)} active users</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-lg">
              ...
              <div className="text-sm">
                ...
                <div className="text-blue-700">Average 78.9% users return after first week</div>
              </div>
            </div>
            <div className="flex items-start gap-2 p-3 bg-green-50 rounded-lg">
              <Users className="h-4 w-4 text-green-600 mt-0.5" />
              <div className="text-sm">
                <div className="font-medium text-green-900">Healthy Long-term Retention</div>
                <div className="text-green-700">35.4% of users stay active after 12 weeks</div>
              </div>
            </div>
            <div className="flex items-start gap-2 p-3 bg-yellow-50 rounded-lg">
              <Mail className="h-4 w-4 text-yellow-600 mt-0.5" />
              <div className="text-sm">
                <div className="font-medium text-yellow-900">Week 2-4 Critical Period</div>
                <div className="text-yellow-700">Focus email campaigns on weeks 2-4 for maximum impact</div>
              </div>
            </div>
            <div className="flex items-start gap-2 p-3 bg-purple-50 rounded-lg">
              <div className="text-sm">
                <div className="font-medium text-purple-900">Cohort Performance Variance</div>
                <div className="text-purple-700">Recent cohorts show 12% better retention rates</div>
              </div>
            </div>
          </div>
        </div>
