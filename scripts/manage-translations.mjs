...
class TranslationManager {
  ...
  validateTranslations() {
    ...
    for (const file of TRANSLATION_FILES) {
      ...
      for (const lang of SUPPORTED_LANGUAGES) {
        ...
        const completeness = referenceKeys.length > 0 ? (translatedKeys.length / referenceKeys.length) * 100 : 0;
        ...
        console.log(`  ${lang}: ${completeness.toFixed(1)}% (${translatedKeys.length}/${referenceKeys.length}) - ${missing} missing`);
      }
    }
  ...
  generateReport() {
    ...
    Array.from(languageStats.entries())
      .sort((a, b) => (b[1].translatedKeys / b[1].totalKeys) - (a[1].translatedKeys / a[1].totalKeys))
      .forEach(([lang, stats]) => {
        const completeness = (stats.translatedKeys / stats.totalKeys) * 100;
        const bar = '█'.repeat(Math.floor(completeness / 5)) + '░'.repeat(20 - Math.floor(completeness / 5));
        console.log(`  ${lang}: ${bar} ${completeness.toFixed(1)}% (${stats.translatedKeys}/${stats.totalKeys})`);
      });
    ...
    TRANSLATION_FILES.forEach(file => {
      ...
      fileResults
        ...
        .forEach(result => {
          const bar = '█'.repeat(Math.floor(result.completeness / 5)) + '░'.repeat(20 - Math.floor(result.completeness / 5));
          console.log(`    ${result.language}: ${bar} ${result.completeness.toFixed(1)}% (${result.missing} missing)`);
        });
    });
    
    console.log('\\n🎯 PRIORITY LANGUAGES TO WORK ON:');
    const priorities = Array.from(languageStats.entries())
      .sort((a, b) => (a[1].translatedKeys / a[1].totalKeys) - (b[1].translatedKeys / b[1].totalKeys))
      .slice(0, 5);
    
    priorities.forEach(([lang, stats], index) => {
      const completeness = (stats.translatedKeys / stats.totalKeys) * 100;
      console.log(`  ${index + 1}. ${lang}: ${completeness.toFixed(1)}% complete (${stats.totalKeys - stats.translatedKeys} keys needed)`);
    });
  }
