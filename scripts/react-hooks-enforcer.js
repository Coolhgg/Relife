...
class ReactHooksEnforcer {
  constructor(options = {}) {
    ...
    this.riskPatterns = [
      ...
      {
        pattern: /setState.*function/,
        risk: 'high',
        message: 'setState in dependency might cause infinite loop',
      },
      {
        pattern: /dispatch.*function/,
        risk: 'medium',
        message: 'dispatch function in dependency - verify if needed',
      },
      {
        pattern: /\.current/,
        risk: 'low',
        message: 'ref.current in dependency - usually not needed',
      },
    ];
  }
  ...
  detectPotentialInfiniteLoops(issue) {
    ...
    if (issue.message.includes('missing dependencies')) {
      ...
      if (depsMatch) {
        const suggestedDeps = depsMatch[1]
          .split(',')
  ...
  generateSmartSuggestions(issue) {
    ...
    if (issue.message.includes('missing dependencies')) {
      ...
      suggestions.push(
        '💡 Consider using useCallback/useMemo for object/array dependencies'
      );
      suggestions.push('🎯 Move constants outside the component to avoid dependencies');

      if (risks.some(r => r.level === 'high')) {
        suggestions.push(
          '⚠️ HIGH RISK: This might cause infinite re-renders - test thoroughly'
        );
    ...
    const hookTypeMatch = issue.message.match(
    ...
    if (hookTypeMatch) {
      ...
      switch (hookType) {
        case 'useEffect':
          suggestions.push(
            '💭 Consider if this effect needs to run on every dependency change'
          );
          break;
        case 'useCallback':
        ...
        case 'useMemo':
          suggestions.push(
            '📊 Ensure the memoization provides actual performance benefit'
          );
          break;
  ...
  async run(files = []) {
    this.log(
      `${colors.bright}🔍 Starting React Hooks Dependency Analysis...${colors.reset}`
    );
    ...
    try {
      ...
      if (hookIssues.length === 0) {
        ...
        this.log(
          `📊 Analyzed ${this.stats.hooksFound} hooks across ${this.stats.filesAnalyzed} files`,
          'cyan'
        );
        return this.stats;
      }
      ...
      hookIssues.forEach(issue => {
        ...
        if (this.options.verbose && issue.hooks.length > 0) {
          ...
          issue.hooks.forEach(hook => {
            this.log(
              `      Line ${hook.line}: ${hook.type} - ${hook.content.substring(0, 60)}...`,
              'blue'
            );
          });
        }
            ...
            const riskColor =
              risk.level === 'high'
                ...
                : risk.level === 'medium'
                  ...
                  : 'blue';
            this.log(
              `   🚨 ${risk.level.toUpperCase()} RISK: ${risk.message}`,
              riskColor
            );
          });
      ...
      if (hookIssues.length > this.options.maxWarnings) {
        this.log(
          `\n... and ${hookIssues.length - this.options.maxWarnings} more issues.`,
          'yellow'
        );
      ...
      if (this.options.strict && this.stats.dependencyIssues > 0) {
        this.log(
          `\n❌ STRICT MODE: Blocking commit due to ${this.stats.dependencyIssues} hook dependency issues`,
          'red'
        );
        process.exit(1);
      }
      ...
      if (this.stats.criticalIssues > 0) {
        this.log(
          `\n⚠️ ${this.stats.criticalIssues} critical hook dependency issues found`,
          'yellow'
        );
        this.log(
          `   These may cause infinite re-renders. Please review carefully.`,
          'yellow'
        );
  ...
  printSummary() {
    ...
    this.log(
      `   Potential infinite loops: ${this.stats.potentialInfiniteLoops}`,
      'red'
    );
  ...
  printRecommendations(issues) {
    ...
    this.log(
      '   3. Use React DevTools Profiler to check for performance issues',
      'blue'
    );
    ...
    if (this.stats.potentialInfiniteLoops > 0) {
      ...
      this.log(
        '   • Consider using useCallback/useMemo for object dependencies',
        'red'
      );
    ...
    this.log(
      '   • Fix all: bunx eslint src/ --fix --rule "react-hooks/exhaustive-deps: error"',
      'cyan'
    );
    this.log(
      '   • Analyze: bunx eslint src/ --rule "react-hooks/exhaustive-deps: error"',
      'cyan'
    );
  }
}
