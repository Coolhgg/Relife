...
const COMPATIBILITY_RULES = {
  ...
  'jest': {
    '30.x': {
      ...
      warning: 'Jest 30.x is not compatible with ts-jest 29.x. Use Jest 29.7.0 with ts-jest 29.2.5'
    }
  },
  'ts-jest': {
    '29.x': {
      ...
      warning: 'ts-jest 29.x requires Jest 29.x. Jest 30.x support is not yet stable'
    }
  }
};
...
class DependencyChecker {
  ...
  checkOutdatedDependencies() {
    ...
    try {
      ...
      try {
        ...
        console.log('ℹ️  Could not parse outdated dependencies (this is normal if all are up to date)');
        return [];
      }
  ...
  checkCurrentDependencies() {
    ...
    if (jestVersion && tsJestVersion) {
      ...
      const tsJestMajor = this.getMajorVersion(tsJestVersion.replace(/[\^~]/, ''));
  ...
  checkIncompatibilities(packageName, version, rule) {
    ...
    for (const incompatible of rule.incompatible) {
      ...
      if (currentDepVersion && this.matchesVersionPattern(currentDepVersion, depVersionPattern)) {
        this.violations.push({
          type: 'incompatibility',
          package: packageName,
          version: version,
          incompatibleWith: incompatible,
          message: rule.warning || `${packageName}@${version} is incompatible with ${incompatible}`,
          severity: 'error'
        });
      }
    }
