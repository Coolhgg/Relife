...
function validateE2ETestResults() {
  ...
  try {
            ...
            test.results.forEach(result => {
              if (result.violations) {
                result.violations.forEach(violation => {
                  ...
                  violations[impact] = (violations[impact] || 0) + (violation.nodes?.length || 1);
                });
              }
            });
          }
    ...
    const exceedsLimits = Object.entries(violations).some(([severity, count]) => 
      count > (A11Y_GATES.maxViolations[severity] || 0)
    );
...
function validateRequiredArtifacts() {
  const missing = A11Y_GATES.requiredArtifacts.filter(artifact => 
...
function generateGateReport(results) {
  ...
  if (report.summary.missingTests > 0) {
    report.status = 'failed';  
    report.recommendations.push('Run accessibility tests and ensure all artifacts are generated');
  }
  
  // Add specific recommendations
  if (results.lighthouse?.score && results.lighthouse.score < A11Y_GATES.minScores.lighthouse) {
    report.recommendations.push(`Improve Lighthouse accessibility score to ${A11Y_GATES.minScores.lighthouse}+`);
  }
  ...
  if (results.e2e?.violations) {
    ...
    if (criticalViolations > 0) {
      report.recommendations.push(`Fix ${criticalViolations} critical accessibility violations`);
    }
    
    if (seriousViolations > 0) {
      report.recommendations.push(`Fix ${seriousViolations} serious accessibility violations`);
    }
  }
...
function printResults(report) {
  ...
  Object.entries(report.results).forEach(([test, result]) => {
    ...
    if (test === 'e2e' && result.violations) {
      ...
      console.log(`      Violations: ${v.critical} critical, ${v.serious} serious, ${v.moderate} moderate, ${v.minor} minor`);
    }
    
    if (test === 'pa11y' && result.violations) {
      const v = result.violations;
      console.log(`      Issues: ${v.errors} errors, ${v.warnings} warnings, ${v.notices} notices`);
    }
  });
...
async function main() {
  ...
  try {
    ...
    if (!requiresTesting) {
      console.log('ℹ️  No accessibility-related changes detected. Skipping accessibility gates.');
      process.exit(EXIT_CODES.SUCCESS);
    }
    
    console.log(`📁 Changed files trigger accessibility testing: ${changedFiles.slice(0, 5).join(', ')}${changedFiles.length > 5 ? '...' : ''}`);
