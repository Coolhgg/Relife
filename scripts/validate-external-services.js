...
function validateEnvVar(env, varName) {
    ...
    if (!value || value.includes('your_') || value.includes('_here') || value.includes('_key_here')) {
        return false;
    }
    return true;
}
        ...
        Object.keys(services).forEach(category => {
            ...
            Object.keys(categoryServices).forEach(serviceName => {
                ...
                if (result.configured === result.total) {
                    ...
                    log.warning(`${serviceName}: ${percentage}% configured (${result.missing.length} missing)`);
