...
function main() {
  const [historyFile, notificationType, criticalIssues, forceNotify] =
  ...
  try {
    ...
    const recentNotifications = history.filter(
      n => new Date(n.timestamp) > dayAgo && n.type === notificationType
    );
    ...
    if (notificationType === 'daily-check' && recentNotifications.length > 0) {
      ...
      if (!hasCritical && !isForceNotify) {
        shouldNotify = false;
        console.log(
          'ℹ️ Suppressing daily notification - sent recently and no critical issues'
        );
      }
    }
