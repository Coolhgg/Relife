...
function main() {
  const [analysisFile, minThreshold, criticalThreshold, notificationType] = process.argv.slice(2);
  
  if (!analysisFile || !minThreshold || !criticalThreshold) {
    console.error('Usage: node analyze-translation-health.js <analysis-file> <min-threshold> <critical-threshold> <notification-type>');
    process.exit(1);
  }
  ...
  try {
    ...
    if (belowThreshold.length > 0) {
      ...
      console.log(`⚠️ Languages below quality threshold (${minQualityThreshold}%): ${belowThreshold.length}`);
    }
    ...
    fs.writeFileSync(process.env.NOTIFICATIONS_DIR + '/notification-data.json', JSON.stringify(notificationData, null, 2));
