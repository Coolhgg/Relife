...
function main() {
  ...
  if (!reportFile || !threshold) {
    console.error('Usage: node analyze-quality-results.js <report-file> <threshold> <monitoring-type>');
    process.exit(1);
  }
  ...
  try {
    ...
    const belowThreshold = languages.filter(([_, lang]) => 
      Math.round(lang.qualityScore?.overall || 0) < qualityThreshold
    ).map(([code]) => code);
    
    console.log(`Average Quality: ${averageQuality}%`);
    console.log(`Languages Below Threshold (${qualityThreshold}%): ${belowThreshold.length}`);
