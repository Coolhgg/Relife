...
class BundleSizeMonitor {
  ...
  async analyzeBundleSize() {
    ...
    if (!fs.existsSync(this.options.distDir)) {
      throw new Error(`Distribution directory '${this.options.distDir}' not found. Run build first.`);
    }
  ...
  findMatchingFiles(pattern) {
    ...
    try {
      return execSync(`find ${this.options.distDir} -name "${path.basename(pattern)}"`, { encoding: 'utf8' })
        .trim()
        .split('\n')
