  ...
  try {
    ...
    violations.forEach(violation => {
      ...
      if (
        ...
        log(
          '   💡 Fix: Ensure hook functions start with "use" (e.g., useMyHook)',
          'yellow'
        );
        log(
          '   💡 Alternative: Move hook calls to the top level of a component',
          'yellow'
        );
      }
      ...
      if (violation.message.includes('called conditionally')) {
        log(
          '   💡 Fix: Move hook calls to the top level, outside conditions/loops',
          'yellow'
        );
      }
    });
    ...
    log(
      '   • Ensure hooks are only called from React components or custom hooks',
      'blue'
    );
