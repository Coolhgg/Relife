...
function main() {
  ...
  try {
    ...
    const good = summary.languageBreakdown.filter(
      l => l.quality >= 60 && l.quality < 80
    );
    ...
    if (excellent.length > 0) {
      coverageReport +=
        excellent.map(l => `- ${l.code.toUpperCase()}: ${l.quality}%`).join('\n') +
        '\n';
    }
    coverageReport += `\n`;
    ...
    if (good.length > 0) {
      coverageReport +=
        good.map(l => `- ${l.code.toUpperCase()}: ${l.quality}%`).join('\n') + '\n';
    }
    coverageReport += `\n`;
    ...
    if (needsWork.length > 0) {
      coverageReport +=
        needsWork.map(l => `- ${l.code.toUpperCase()}: ${l.quality}%`).join('\n') +
        '\n';
    }
