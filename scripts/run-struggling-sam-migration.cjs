...
async function runMigration() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    ...
    console.error('Please ensure VITE_SUPABASE_URL and SUPABASE_SERVICE_KEY are set in your environment');
    process.exit(1);
  }
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../database/struggling-sam-migration.sql');
      ...
      try {
        ...
        if (error) {
          ...
          if (altError && statement.includes('CREATE TABLE')) {
            console.log('⚠️  Statement may have executed (CREATE statements not directly verifiable)');
            successCount++;
          } else if (altError) {
            throw error;
          } else {
        ...
        if (statementError.message.includes('already exists')) {
          console.log('ℹ️  This is expected if running migration multiple times');
          successCount++;
          errorCount--;
        }
      }
...
async function verifyMigration(supabase) {
  ...
  if (verifiedTables === tablesToCheck.length) {
    ...
    console.log('⚠️  Some tables could not be verified. Please check your database configuration.');
  }
}
