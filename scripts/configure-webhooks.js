...
class WebhookConfigurationManager {
  ...
  async configureStripeWebhooks() {
    ...
    console.log('3. Enter your webhook URL: https://yourdomain.com/api/stripe/webhooks');
    ...
    const webhookSecret = await question('Enter your Stripe Webhook Secret (whsec_...): ');
    if (!webhookSecret.startsWith('whsec_')) {
      log.error('Invalid webhook secret format');
      return;
    }
    
    const webhookUrl = await question('Enter your webhook URL (https://yourdomain.com/api/stripe/webhooks): ');
  ...
  async configurePushNotifications() {
    ...
    const vapidPublicKey = await question('Enter VAPID Public Key for web push (optional): ');
    const vapidPrivateKey = await question('Enter VAPID Private Key for web push (optional): ');
    
    // Push notification settings
    const pushSettings = {
      alarmReminders: await question('Enable alarm reminder notifications? (y/n): ') === 'y',
      dailyMotivation: await question('Enable daily motivation messages? (y/n): ') === 'y',
      weeklyProgress: await question('Enable weekly progress reports? (y/n): ') === 'y',
  ...
  async configureMonitoringWebhooks() {
    ...
    const configurePagerDuty = await question('Configure PagerDuty notifications? (y/n): ');
  ...
  async generateEnvironmentFiles() {
    ...
    try {
      ...
      await fs.writeFile(path.join(this.projectRoot, 'webhook-configuration-summary.md'), summary);
...
`;
  ...
  async testStripeWebhook() {
    try {
      ...
      return this.config.stripe.secretKey?.startsWith('sk_') && 
  ...
  async testDatabaseConnection() {
    try {
      ...
      return this.config.database?.supabaseUrl?.includes('supabase.co') &&
  ...
  async performSecurityAudit() {
    ...
    if (this.config.stripe?.webhookUrl && !this.config.stripe.webhookUrl.startsWith('https://')) {
  ...
  async viewCurrentConfiguration() {
    ...
    console.log(`  Stripe: ${this.config.stripe?.secretKey ? '✅ Configured' : '❌ Not Configured'}`);
    console.log(`  Push Notifications: ${this.config.pushNotifications?.enabled ? '✅ Enabled' : '❌ Disabled'}`);
    console.log(`  Monitoring: ${Object.keys(this.config.monitoring || {}).length} services configured`);
    console.log(`  Database: ${this.config.database?.supabaseUrl ? '✅ Configured' : '❌ Not Configured'}`);
    console.log(`  Security: ${this.config.security?.webhookAuthToken ? '✅ Configured' : '❌ Not Configured'}`);
