...
class CorruptionDetector {
  ...
  detectEscapedNewlines(filePath, content) {
    ...
    patterns.forEach((pattern, index) => {
      ...
      if (matches && matches.length > 0) {
        const patternNames = ['escaped newlines', 'escaped tabs', 'escaped carriage returns', 'escaped double quotes', 'escaped single quotes', 'escaped greater than', 'escaped less than'];
        issues.push({
  ...
  detectEncodingIssues(filePath) {
    try {
      ...
      const result = execSync(`file -b "${filePath}"`, { encoding: 'utf8' }).trim();
      ...
      try {
        ...
        if (readError.code === 'EILSEQ' || readError.message.includes('invalid')) {
          this.results.encodingIssues.push({
            file: filePath,
  ...
  detectFormatIssues(filePath, content) {
    ...
    const linesWithTrailingWhitespace = content.split('\n').filter(line => line.match(/\s+$/));
    if (linesWithTrailingWhitespace.length > 0) {
      issues.push(`Trailing whitespace on ${linesWithTrailingWhitespace.length} lines`);
    }
    ...
    if (tabIndented > 0 && spaceIndented > 0) {
      issues.push(`Mixed indentation: ${tabIndented} tab-indented, ${spaceIndented} space-indented`);
    }
  ...
  run(rootDir = '.') {
    ...
    console.log('\nBreakdown:');
    console.log(`  Escaped newlines: ${report.categories.escapedNewlines.count} files`);
    console.log(`  Compressed files: ${report.categories.compressedFiles.count} files`);
    console.log(`  Encoding issues: ${report.categories.encodingIssues.count} files`);
    console.log(`  JSX syntax breakage: ${report.categories.jsxSyntaxBreakage.count} files`);
    console.log(`  Format issues: ${report.categories.formatIssues.count} files`);
