...
const alarmTimes = new SharedArray('alarm_times', function () {
  return [
    ...
    '09:00', '21:00', '21:30', '22:00', '22:30', '23:00'
  ];
});

const alarmLabels = new SharedArray('alarm_labels', function () {
  return [
    'Morning Workout', 'Work Start', 'Meeting Reminder', 'Lunch Break',
    'Afternoon Break', 'End of Work', 'Dinner Time', 'Evening Walk',
    'TV Show', 'Bedtime Routine', 'Medication', 'Water Break'
  ];
});
...
const AUTH_TOKEN = __ENV.AUTH_TOKEN || 'test-auth-token-' + Math.random().toString(36).substr(2, 9);
...
export function alarmLifecycleTest() {
  ...
  const authResponse = http.post(`${API_URL}/auth/validate`, JSON.stringify({
    token: AUTH_TOKEN,
    userId: userId,
  }), { 
    headers: headers,
  ...
  const createAlarmResponse = http.post(`${API_URL}/alarms`, JSON.stringify({
    time: alarmTime,
    label: alarmLabel,
    enabled: true,
  ...
  }) ? createAlarmResponse.json('id') || `alarm_${Math.random().toString(36).substr(2, 9)}` : null;
  ...
  if (alarmId) {
    ...
    const dismissResponse = http.post(`${API_URL}/alarms/${alarmId}/dismiss`, JSON.stringify({
    ...
    const analyticsResponse = http.post(`${API_URL}/analytics/events`, JSON.stringify({
