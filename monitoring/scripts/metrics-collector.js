...
const collectMobileMetrics = async () => {
    try {
        ...
        info(`Collected ${Object.keys(mobileData).length * 2} mobile metrics (iOS + Android)`);
...
const main = async () => {
    ...
    if (CONFIG.enabled.performance) {
        ...
        info(`Performance metrics collection started (every ${CONFIG.intervals.performance}s)`);
    }
