...
const DEFAULT_THEMES: Record<Theme, ThemeConfig> = {
  ...
  colorful: {
    ...
    effects: {
      ...
      gradients: {
        primary:
          "linear-gradient(135deg, #ec4899 0%, #f472b6 50%, #38bdf8 100%)",
        secondary:
  ...
  retro: {
    ...
    effects: {
      ...
      gradients: {
        ...
        background:
          "linear-gradient(135deg, #1a0033 0%, #330066 50%, #001133 100%)",
      },
    },
  },
  cyberpunk: {
    ...
    effects: {
      ...
      gradients: {
        ...
        background:
          "linear-gradient(135deg, #000011 0%, #110022 50%, #220011 100%)",
      },
    },
  },
  spring: {
...
export function ThemeProvider({
  ...
  useEffect(() => {
    const initializeThemeData = async () => {
      try {
        ...
        if (
          enableSystem &&
          (!themeData.theme || themeData.theme === "system")
        ) {
        ...
        if (
          ...
          setThemeConfig(
            themeData.themeConfig || DEFAULT_THEMES[selectedTheme],
          );
        }
        ...
        if (
          storedTheme &&
          Object.keys(DEFAULT_THEMES).includes(storedTheme as Theme)
        ) {
  ...
  const saveThemePreset = useCallback(
    async (preset: ThemePreset): Promise<void> => {
      ...
      if (persistenceServiceRef.current) {
        ...
        if (success) {
          ...
          if (themeData.theme) {
            ...
            setThemeConfig(
              themeData.themeConfig || DEFAULT_THEMES[themeData.theme],
            );
          }
  ...
  const syncThemes = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      await cloudSyncServiceRef.current.updatePreferences({
        ...
        deviceId: cloudSyncServiceRef.current.getStatus().isOnline
          ? "web"
          : "offline",
        version: 1,
      });
      ...
      const updatedPreferences =
        await cloudSyncServiceRef.current.getPreferences();
      if (updatedPreferences) {
        ...
        if (updatedPreferences.theme !== theme) {
          ...
          setThemeConfig(
            updatedPreferences.themeConfig ||
              DEFAULT_THEMES[updatedPreferences.theme],
          );
        }
  ...
  const getCSSVariables = useMemo((): Record<string, string> => {
    ...
    const fontSizeScale =
      personalization.typographyPreferences.fontSizeScale || 1;
    ...
    const animationScale = personalization.motionPreferences.enableAnimations
      ? 1
      : 0;
  ...
  const calculateContrastRatio = useCallback(
    (foreground: string, background: string) => {
      const accessibilityService = ThemeAccessibilityService.getInstance();
      return accessibilityService.calculateContrastRatio(
        foreground,
        background,
      );
    },
  ...
  const getDefaultAnimationEffects =
    useCallback((): PremiumAnimationEffects => {
