...
function useSubscription(
  ...
  const comparePlans = useCallback(
    (currentTier: SubscriptionTier, targetTier: SubscriptionTier) => {
      ...
      const currentPlan = state.availablePlans.find((p: any) => // auto: implicit any p.tier === currentTier);
      const targetPlan = state.availablePlans.find((p: any) => // auto: implicit any p.tier === targetTier);
