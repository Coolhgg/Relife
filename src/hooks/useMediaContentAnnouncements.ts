...
export function useMediaContentAnnouncements() {
  ...
  const announceTabChange = useCallback(
    (tabName: string) => {
      const tabDescriptions: Record<string, string> = {
        sounds:
          "Sounds tab selected. Browse and manage audio content for alarms.",
        playlists:
          "Playlists tab selected. Create and manage custom sound playlists.",
        quotes: "Quotes tab selected. Browse and submit motivational quotes.",
        challenges:
          "Photo challenges tab selected. Complete photo challenges for rewards.",
      };
  ...
  const announceSearchResults = useCallback(
    (query: string, resultCount: number) => {
      if (query.trim() === "") {
        ...
        announce(
          `Found ${resultCount} sound${resultCount === 1 ? "" : "s"} matching "${query}".`,
          "polite",
        );
      }
    },
  ...
  const announcePlaylistAction = useCallback(
    (
      action: "created" | "updated" | "deleted" | "played",
      playlist: Playlist,
    ) => {
      let message = "";
  ...
  const announceQuoteAction = useCallback(
    (
      action: "submitted" | "liked" | "used" | "browsing",
  ...
  const announcePhotoChallengeAction = useCallback(
    (
  ...
  const announceShare = useCallback(
    (contentType: "sound" | "playlist" | "quote", contentName: string) => {
      announce(
        `${contentType} "${contentName}" shared successfully!`,
        "polite",
      );
    },
    [announce],
  ...
  const announceDetailedPlaylistInfo = useCallback(
    (playlist: Playlist) => {
      ...
      const soundNames = playlist.sounds
        .slice(0, 3)
        .map((s) => s.sound.name)
        .join(", ");
