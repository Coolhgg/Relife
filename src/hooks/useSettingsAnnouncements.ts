...
export function useSettingsAnnouncements() {
  ...
  const announceSliderChange = useCallback((sliderName: string, value: number, min: number, max: number, label?: string) => {
  ...
  const announceSettingDescription = useCallback((settingName: string, currentValue: string, description: string) => {
    announce({
      type: 'custom',
  ...
  const announceSettingsLoading = useCallback((action: string, isLoading: boolean) => {
    if (isLoading) {
      announce({
        type: 'custom',
  ...
  const announceLinkActivation = useCallback((linkName: string, opensInNewWindow: boolean = false) => {
