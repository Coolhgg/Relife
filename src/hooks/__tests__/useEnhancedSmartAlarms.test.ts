...
describe("useEnhancedSmartAlarms", () => {
  ...
  it("should generate smart recommendations", async () => {
    ...
    expect(
      mockSmartAlarmScheduler.generateSmartRecommendations,
  ...
  it("should handle user feedback and adapt", async () => {
    ...
    expect(mockSmartAlarmScheduler.adaptToUserFeedback).toHaveBeenCalledWith(
      feedback,
    );
    expect(mockAnalytics.track).toHaveBeenCalledWith(
      "smart_alarm_feedback",
      feedback,
    );
  });
  ...
  it("should get optimization metrics", async () => {
    ...
    expect(
      mockSmartAlarmScheduler.getOptimizationMetrics,
  ...
  it("should handle AI-based wake preference predictions", async () => {
    ...
    await act(async () => {
      const preferences =
        await result.current.predictWakePreferences("user-123");
    ...
    expect(mockAIOptimizer.predictWakePreferences).toHaveBeenCalledWith(
      "user-123",
    );
  });
  ...
  it("should reset learning data when requested", async () => {
    ...
    expect(mockSmartAlarmScheduler.resetLearningData).toHaveBeenCalledWith(
      "user-123",
    );
  ...
  it("should batch multiple optimization requests", async () => {
    ...
    expect(mockSmartAlarmScheduler.optimizeAlarmTiming).toHaveBeenCalledTimes(
      3,
    );
  });
});
