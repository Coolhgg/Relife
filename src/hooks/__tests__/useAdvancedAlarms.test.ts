...
import { renderHookWithProviders, createMockAlarm, clearAllMocks } from '../../__tests__/utils/hook-testing-utils';
...
describe('useAdvancedAlarms Hook', () => {
  ...
  beforeEach(() => {
    ...
    AdvancedAlarmScheduler.applySmartOptimizations.mockImplementation((alarm) => Promise.resolve(alarm));
    AdvancedAlarmScheduler.applySeasonalAdjustments.mockImplementation((alarm) => alarm);
    AdvancedAlarmScheduler.calculateNextOccurrences.mockReturnValue([new Date(Date.now() + 24 * 60 * 60 * 1000)]);
    ...
    AdvancedAlarmScheduler.exportSchedule.mockResolvedValue({ alarms: [mockAdvancedAlarm] });
    AdvancedAlarmScheduler.importSchedule.mockResolvedValue({ success: 1, failed: 0, errors: [] });
  ...
  describe('Initialization', () => {
    ...
    it('should handle scheduler initialization errors', async () => {
      const AdvancedAlarmScheduler = require('../../services/advanced-alarm-scheduler').default;
      AdvancedAlarmScheduler.initialize.mockRejectedValue(new Error('Scheduler failed'));
  ...
  describe('Creating Alarms', () => {
    it('should create advanced alarm successfully', async () => {
      ...
      expect(AdvancedAlarmScheduler.scheduleAdvancedAlarmNotifications).toHaveBeenCalled();
    });
  ...
  describe('Updating Alarms', () => {
    it('should update alarm successfully', async () => {
      ...
      const AdvancedAlarmScheduler = require('../../services/advanced-alarm-scheduler').default;
      expect(AdvancedAlarmScheduler.cancelAdvancedAlarmNotifications).toHaveBeenCalledWith(mockBasicAlarm.id);
      expect(AdvancedAlarmScheduler.scheduleAdvancedAlarmNotifications).toHaveBeenCalled();
    });
  ...
  describe('Import/Export', () => {
    ...
    it('should handle export errors', async () => {
      const AdvancedAlarmScheduler = require('../../services/advanced-alarm-scheduler').default;
      AdvancedAlarmScheduler.exportSchedule.mockRejectedValue(new Error('Export failed'));
  ...
  describe('Scheduling Features', () => {
    it('should handle conditional rules evaluation', async () => {
      ...
      expect(AdvancedAlarmScheduler.scheduleAdvancedAlarmNotifications).not.toHaveBeenCalled();
    });
