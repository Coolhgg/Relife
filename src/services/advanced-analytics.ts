...
class AdvancedAnalyticsService {
  ...
  private insightCache = new Map<string, { data: AnalyticsInsight[]; timestamp: number }>();
  private analyticsCache = new Map<string, { data: UserAnalytics; timestamp: number }>();
   ...
   */
  async generateUserAnalytics(userId: string, period: UserAnalytics['period'] = 'month'): Promise<UserAnalytics> {
    try {
      ...
      const summary = await this.calculateSummaryMetrics(alarmData, sleepData, eventsData);

      // Generate trends
      const trends = await this.calculateTrends(userId, period, eventsData, sleepData);

      // Generate insights
      const insights = await this.generateInsights(userId, summary, trends, alarmData, sleepData, eventsData);

      // Perform comparative analysis
      const comparative_analysis = await this.performComparativeAnalysis(userId, summary, period);

      // Build prediction models
      const prediction_models = await this.buildPredictionModels(userId, eventsData, sleepData, trends);
      ...
      this.performanceMonitor.trackCustomMetric('advanced_analytics_generation', duration);
   ...
   * Calculate summary metrics
   */
  private async calculateSummaryMetrics(alarmData: any[], sleepData: any[], eventsData: any[]): Promise<UserAnalytics['summary']> {
    const total_alarms = eventsData.length;
    const successful_wake_ups = eventsData.filter(event => event.dismissed && !event.snoozed).length;
    const average_response_time = eventsData
    ...
    const success_rate = total_alarms > 0 ? (successful_wake_ups / total_alarms) * 100 : 0;
   ...
   */
  private async calculateTrends(userId: string, period: string, eventsData: any[], sleepData: any[]): Promise<UserAnalytics['trends']> {
    const wake_time_consistency = await this.calculateWakeTimeConsistencyTrend(eventsData);
    const response_time_trend = await this.calculateResponseTimeTrend(eventsData);
    const voice_mood_effectiveness = await this.calculateVoiceMoodEffectivenessTrend(userId, eventsData);
  ...
  private async performComparativeAnalysis(
    ...
    const peer_comparison = await this.calculatePeerComparison(summary.success_rate, 'success_rate');
  ...
  private async buildPredictionModels(
    ...
    const sleep_recommendations = await this.generateSleepRecommendations(sleepData, eventsData);
  ...
  async getRealtimeDashboard(userId: string): Promise<{
    ...
    try {
      ...
      // Get today's events
      const { data: todayEvents, error } = await this.supabaseService.getInstance().client
        .from('alarm_events')
        .select(`
          *,
      ...
      const liveMetrics = {
        ...
        successfulWakeups: todayEvents?.filter((e: { dismissed: boolean; snoozed: boolean }) => e.dismissed && !e.snoozed).length || 0,
      ...
      const todayStats = {
        ...
        totalSnoozed: todayEvents?.filter((e: { snoozed: boolean }) => e.snoozed).length || 0
      };
      ...
      const quickActions = await this.generateQuickActions(userId, liveMetrics, activeInsights);
  ...
  private calculateConsistencyScore(eventsData: any[]): number {
    ...
    const variance = wakeTimes.reduce((sum, time) => sum + Math.pow(time - mean, 2), 0) / wakeTimes.length;
  ...
  private calculateSleepHealthScore(sleepData: any[]): number {
    ...
    const avgDuration = recentSleep.reduce((sum, session) => sum + session.total_duration, 0) / recentSleep.length;
    const avgEfficiency = recentSleep.reduce((sum, session) => sum + (session.sleep_efficiency || 80), 0) / recentSleep.length;
    const avgQuality = recentSleep.reduce((sum, session) => sum + (session.restfulness_score || 70), 0) / recentSleep.length;
    ...
    const healthScore = (durationScore * 0.4) + (efficiencyScore * 0.3) + (qualityScore * 0.3);
    return Math.round(healthScore);
  }

  private async calculateWakeTimeConsistencyTrend(eventsData: any[]): Promise<TrendData> {
  ...
  private async calculateVoiceMoodEffectivenessTrend(userId: string, eventsData: any[]): Promise<TrendData> {
  ...
  private calculateTrendDirection(values: { date: string; value: number }[]): 'up' | 'down' | 'stable' {
  ...
  private calculateStatisticalSignificance(values: { date: string; value: number }[]): number {
  ...
  private async getAlarmData(userId: string, period: string): Promise<any[]> {
    ...
    const { data, error } = await this.supabaseService.getInstance().client
      .from('alarms')
      .select('*')
      .eq('user_id', userId)
  ...
  private async getSleepData(userId: string, period: string): Promise<any[]> {
    ...
    const { data, error } = await this.supabaseService.getInstance().client
      ...
      .select('*')
      .eq('user_id', userId)
      .gte('sleep_start', new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000).toISOString())
  ...
  private async getEventData(userId: string, period: string): Promise<any[]> {
    ...
    const { data, error } = await this.supabaseService.getInstance().client
      ...
      `)
      .eq('alarms.user_id', userId)
      .gte('fired_at', new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000).toISOString())
      .order('fired_at', { ascending: false });
  ...
  private async analyzeVoiceEffectiveness(userId: string, eventsData: any[]): Promise<any> {
    ...
    return {
      ...
      confidence: Math.min(90, Math.max(50, moodEffectiveness.get(bestMood)?.total * 10 || 50))
    };
  }

  private async calculatePeerComparison(userValue: number, metric: string): Promise<PeerComparison> {
  ...
  private async calculatePersonalBest(userId: string, metric: string): Promise<PersonalBest> {
    // Query historical best performance
    const { data, error } = await this.supabaseService.getInstance().client
      .from('user_analytics_history')
      .select('*')
      .eq('user_id', userId)
      .eq('metric', metric)
  ...
  private async predictOptimalWakeTimes(userId: string, eventsData: any[]): Promise<OptimalWakeTime[]> {
    ...
    for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
      ...
      optimal.push({
        ...
        confidence: Math.min(95, Math.max(60, hourSuccess.get(bestHour)?.total * 20 || 60)),
        expected_success_rate: Math.round(bestRate * 100),
        factors: ['Historical performance', 'Day of week pattern', 'Response time analysis']
      });
    }
  ...
  private async forecastEffectiveness(eventsData: any[], trends: UserAnalytics['trends']): Promise<EffectivenessForecast> {
    const recentEvents = eventsData.slice(0, 14); // Last 2 weeks
    const currentRate = recentEvents.length > 0 ?
      (recentEvents.filter(e => e.dismissed && !e.snoozed).length / recentEvents.length) * 100 : 70;
      ...
      contributing_factors: [
        {
          ...
          current_trend: trends.response_time_trend.trend_direction === 'down' ? 'positive' : 'negative'
        },
        {
          factor: 'Sleep quality trend',
          influence_weight: 0.3,
          current_trend: trends.sleep_quality_trend.trend_direction === 'up' ? 'positive' : 'neutral'
        },
        {
          factor: 'Consistency pattern',
          influence_weight: 0.3,
          current_trend: trends.wake_time_consistency.trend_direction === 'up' ? 'positive' : 'neutral'
        }
      ]
    };
  }

  private async generateSleepRecommendations(sleepData: any[], eventsData: any[]): Promise<SleepRecommendation[]> {
    const recommendations: SleepRecommendation[] = [];

    if (sleepData.length > 0) {
      const avgDuration = sleepData.reduce((sum, session) => sum + session.total_duration, 0) / sleepData.length;
      ...
      if (avgDuration < 420) { // Less than 7 hours
        recommendations.push({
          ...
          rationale: 'Increasing sleep duration can improve morning alertness and alarm responsiveness',
          difficulty: 'moderate'
        });
      }

      const avgEfficiency = sleepData.reduce((sum, session) => sum + (session.sleep_efficiency || 80), 0) / sleepData.length;
      ...
      if (avgEfficiency < 85) {
        recommendations.push({
          ...
          rationale: 'Consistent sleep schedule improves sleep efficiency and wake-up success',
  ...
  private calculateAverageResponseTime(events: any[]): number {
    ...
    return withResponseTime.length > 0 ?
      Math.round(withResponseTime.reduce((sum, e) => sum + e.response_time, 0) / withResponseTime.length) : 0;
  }
  ...
  private async generateQuickActions(userId: string, liveMetrics: any, insights: AnalyticsInsight[]): Promise<any[]> {
    const actions = [];
