...
export class AIRewardsService {
  ...
  private generatePersonalityProfile(
    ...
    const primaryNiche = this.inferPrimaryNiche(
      traits,
      frequencyData,
      morningPerson,
    );
  ...
  private generateNicheProfile(
    ...
    const sortedNiches = Object.entries(nicheScores).sort(
      ([, a], [, b]) => b - a,
    ) as [UserNiche["primary"], number][];
  ...
  private personalizeReward(
    ...
    const personalizedMessages = {
      fitness:
        "Your dedication to fitness is inspiring! Keep pushing those limits! 💪",
    ...
    const personalizedMessage =
      Object.entries(personalizedMessages).find(([type]) =>
        key.includes(type),
      )?.[1] || template.personalizedMessage;
