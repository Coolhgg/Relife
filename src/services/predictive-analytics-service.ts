...
export class PredictiveAnalyticsService {
  ...
  static async analyzeUserPatterns(
    ...
    try {
      ...
      patterns.push(
        ...(await this.detectLocationInfluencePatterns(userId, behaviors)),
      );
      patterns.push(
        ...(await this.detectWeatherSensitivityPatterns(behaviors)),
      );
      patterns.push(
        ...(await this.detectCalendarCorrelationPatterns(behaviors)),
      );
      patterns.push(
        ...(await this.detectSleepQualityImpactPatterns(behaviors)),
      );
      patterns.push(...(await this.detectEnergyLevelPatterns(behaviors)));
      patterns.push(
        ...(await this.detectDifficultyOptimizationPatterns(behaviors)),
      );
  ...
  private static async detectWakeConsistencyPatterns(
    ...
    for (const [day, dayBehaviors] of Object.entries(dayGroups)) {
      ...
      const wakeTimes = dayBehaviors.map((b) =>
        this.timeToMinutes(b.actualWakeTime),
      );
      const avgTime =
  ...
  private static async detectSnoozeBehaviorPatterns(
    ...
    const conditions = [
      `Snooze frequency: ${Math.round(snoozeFrequency * 100)}%`,
    ];
  ...
  private static async detectSeasonalPatterns(
    ...
    for (const [month, monthBehaviors] of Object.entries(monthGroups)) {
      const wakeTimes = monthBehaviors.map((b) =>
        this.timeToMinutes(b.actualWakeTime),
      );
    ...
    const maxVariation = Math.max(
      ...times.map((time) => Math.abs(time - avgTime)),
    );

    if (maxVariation > 20) {
      // 20+ minute seasonal variation
      const winterMonths = [11, 0, 1]; // Dec, Jan, Feb
      const summerMonths = [5, 6, 7]; // Jun, Jul, Aug

      const winterAvg = this.calculateSeasonalAverage(
        monthlyAverages,
        winterMonths,
      );
      const summerAvg = this.calculateSeasonalAverage(
        monthlyAverages,
        summerMonths,
      );
    ...
    try {
      ...
      if (homePattern && workPattern) {
        ...
        const homeBehaviors = behaviors.filter(
          (b) =>
            new Date(b.date).getDay() === 0 || new Date(b.date).getDay() === 6,
        );
        const workBehaviors = behaviors.filter(
          (b) =>
            new Date(b.date).getDay() >= 1 && new Date(b.date).getDay() <= 5,
        );
  ...
  private static async detectWeatherSensitivityPatterns(
    ...
    const rainyDays = behaviors.filter((b) =>
      b.context?.weather?.includes("rain"),
    );
    const sunnyDays = behaviors.filter((b) =>
      b.context?.weather?.includes("sun"),
    );
  ...
  private static calculateInsightPriority(insight: PredictiveInsight): number {
    ...
    return (
      priorityScores[insight.priority] * 2 +
  ...
  private static calculateTrend(
    ...
    const recent =
      values.slice(-3).reduce((sum, v) => sum + v, 0) /
  ...
  private static findConsistencyAnomalies(
    scores: Record<string, number>,
  ): string[] {
    ...
    for (const [day, score] of Object.entries(scores)) {
      if (score < avgScore - 0.3) {
        anomalies.push(
          `${this.getDayName(parseInt(day))} shows lower consistency`,
        );
      }
    }
  ...
  private static calculateSeasonalAverage(
    ...
    return (
      seasonalTimes.reduce((sum, time) => sum + time, 0) / seasonalTimes.length
    );
  }
  ...
  static getRecentInsights(days: number = 7): PredictiveInsight[] {
    ...
    return this.insightsHistory.filter(
      (insight) => insight.createdAt >= cutoff,
    );
  }
