...
export class CapacitorEnhancedService {
        ...
        hasHaptics:
          ...
          (deviceInfo.platform === "ios" ||
            (deviceInfo.platform === "android" &&
              parseInt(deviceInfo.osVersion) >= 8)),
  ...
  private async initializeNetwork(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[Capacitor] Network monitoring initialization failed:",
        error,
      );
    }
  }
  ...
  async getPendingAlarms(): Promise<LocalNotificationDescriptor[]> {
    ...
    try {
      const pending = await LocalNotifications.getPending();
      return pending.notifications.filter((n) => n.extra?.type === "alarm");
    } catch (error) {
    ...
    if (listeners) {
      listeners.forEach((callback) => {
        try {
          ...
          console.error(
            `[Capacitor] Event listener error for ${event}:`,
            error,
          );
        }
      });
