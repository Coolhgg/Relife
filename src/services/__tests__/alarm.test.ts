...
describe("AlarmService", () => {
  ...
  beforeEach(() => {
    ...
    (AppAnalyticsService.getInstance as jest.Mock).mockReturnValue(
      mockAnalytics,
    );
  ...
  describe("loadAlarms", () => {
    it("should load alarms from secure storage successfully", async () => {
      ...
      expect(mockSecureStorage.retrieveAlarms).toHaveBeenCalledWith(
        mockUser.id,
      );
    ...
    it("should filter invalid alarms during loading", async () => {
      ...
      expect(
        result.every((alarm) => alarm.id && alarm.time && alarm.label),
      ).toBe(true);
    });

    it("should throw error on storage failures", async () => {
      mockSecureStorage.retrieveAlarms.mockRejectedValue(
  ...
  describe("saveAlarms", () => {
    ...
    it("should handle storage errors during save", async () => {
      mockSecureStorage.storeAlarms.mockRejectedValue(
        new Error("Storage error"),
      );
  ...
  describe("toggleAlarm", () => {
    ...
    it("should handle non-existent alarm", async () => {
      await expect(
        AlarmService.toggleAlarm("non-existent", true),
      ).rejects.toThrow("Alarm not found");
    });
    ...
    it("should validate alarm data after toggle", async () => {
      ...
      jest
        .spyOn(AlarmService as any, "validateAlarmData")
        .mockReturnValue(false);

      await expect(
        AlarmService.toggleAlarm(existingAlarm.id, true),
      ).rejects.toThrow("Invalid alarm data after toggle");
    });
  });
  ...
  describe("createBattleAlarm", () => {
    ...
    it("should validate battle alarm data", async () => {
      ...
      await expect(
        AlarmService.createBattleAlarm(battleAlarmData),
      ).rejects.toThrow("Invalid battle alarm data");
    });
  });
  ...
  describe("getUserAlarms", () => {
    ...
    it("should return alarms for specific user", () => {
      ...
      expect(
        result.every((alarm) => !alarm.userId || alarm.userId === mockUser.id),
      ).toBe(true);
    });
...
describe("enhancedAlarmTracking", () => {
  ...
  describe("trackAlarmPerformance", () => {
    it("should track alarm performance for valid alarm", async () => {
      ...
      await enhancedAlarmTracking.trackAlarmPerformance(
        mockAlarm.id,
        mockUser.id,
      );
  ...
  describe("trackBattleAlarmUsage", () => {
    beforeEach(() => {
      ...
      jest
        .spyOn(AlarmService, "getBattleAlarms")
        .mockReturnValue([battleAlarm]);
      jest
        .spyOn(AlarmService, "getNonBattleAlarms")
        .mockReturnValue([regularAlarm]);
    });
