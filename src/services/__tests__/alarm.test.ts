...
vi.mock('../../utils', (
  ...
  generateAlarmId: vi.fn((
) => 'alarm_test_12345'),
  getNextAlarmTime: vi.fn((
) => new Date('2024-01-02T07:00:00.000Z')),
...
jest.mock('../secure-alarm-storage', (
  ...
  default: {
    getInstance: jest.fn((
      ...
      retrieveAlarms: jest.fn((
) => Promise.resolve([])),
      storeAlarmEvents: jest.fn((
) => Promise.resolve()),
      retrieveAlarmEvents: jest.fn((
) => Promise.resolve([])),
    })),
  },
}));
