...
describe("VoiceService", () => {
  ...
  describe("generateAlarmMessage", () => {
    ...
    it("should handle premium voice moods by delegating to PremiumVoiceService", async () => {
      ...
      (PremiumVoiceService.isPremiumPersonality as jest.Mock).mockReturnValue(
        true,
      );
      ...
      const result = await VoiceService.generateAlarmMessage(
        premiumAlarm,
        userId,
      );
    ...
    it("should generate different messages for different voice moods", async () => {
      ...
      const gentleResult = await VoiceService.generateAlarmMessage(
        gentleAlarm,
        userId,
      );
      const drillResult = await VoiceService.generateAlarmMessage(
        drillAlarm,
        userId,
      );
    ...
    it("should handle different alarm times appropriately", async () => {
      ...
      const earlyResult = await VoiceService.generateAlarmMessage(
        earlyAlarm,
        userId,
      );
      const lateResult = await VoiceService.generateAlarmMessage(
        lateAlarm,
        userId,
      );
  ...
  describe("preloadAlarmMessages", () => {
    ...
    it("should preload voice messages for multiple alarms", async () => {
      ...
      expect(mockSpeechSynthesis.speak).toHaveBeenCalledTimes(
        testAlarms.length,
      );
    });
  ...
  describe("integration scenarios", () => {
    ...
    it("should handle complete alarm workflow with voice", async () => {
      ...
      const voiceMessage = await VoiceService.generateAlarmMessage(
        alarm,
        userId,
      );
