...
export class AdvancedConditionsHelper {
  ...
  static async validateConditionSetup(
    ...
    const enabledConditions = conditions.filter((c: any) => c // auto: implicit any.isEnabled).length;
    ...
    const conditionTypes = new Set(
      conditions.filter((c: any) => c // auto: implicit any.isEnabled).map((c: any) => c // auto: implicit any.type)
    );
  ...
  static async analyzeConditionPerformance(
    ...
    const topPerformers = conditions
      .filter((c: any) => c // auto: implicit any.effectivenessScore >= 0.8)
      .map((c: any) => c // auto: implicit any.id);

    const underPerformers = conditions
      .filter((c: any) => c // auto: implicit any.effectivenessScore < 0.5)
      .map((c: any) => c // auto: implicit any.id);
    ...
    const conditionBreakdown = {
      excellent: conditions.filter((c: any) => c // auto: implicit any.effectivenessScore >= 0.9).length,
      ...
      poor: conditions.filter((c: any) => c // auto: implicit any.effectivenessScore < 0.5).length,
    };
  ...
  static async optimizeConditions(alarmId: string): Promise<void> {
    ...
    const optimizedConditions = conditions.map((condition: any) => { // auto: implicit any
