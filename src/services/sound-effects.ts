...
class SoundEffectsService {
      ...
      const mockCommunityThemes: CustomSoundTheme[] = [
        {
          ...
          version: '1.0',
          category: 'nature',
          tags: ['forest', 'rain', 'peaceful', 'ambient'],
          rating: 4.7,
          downloads: 1247,
          popularity: 0.92,
          sounds: {
            ui: {
              click: { type: 'built-in', soundId: 'ui.click' },
              hover: { type: 'built-in', soundId: 'ui.hover' },
              success: { type: 'built-in', soundId: 'ui.success' },
              error: { type: 'built-in', soundId: 'ui.error' }
            },
            notifications: {
              default: { type: 'built-in', soundId: 'notification.default' },
              alarm: { type: 'built-in', soundId: 'notification.alarm' },
              beep: { type: 'built-in', soundId: 'notification.beep' }
            },
            alarms: {
              gentle: { type: 'built-in', soundId: 'alarm.gentle_bells' },
              energetic: { type: 'built-in', soundId: 'alarm.morning_birds' },
              classic: { type: 'built-in', soundId: 'alarm.classic_beep' }
            }
          },
          metadata: {
            author: 'Nature Lover',
          ...
          permissions: {
            ...
            canEdit: 'private',
            canShare: 'registered',
            canDownload: 'registered',
            canRate: 'registered',
            canComment: 'registered'
          },
          isPremium: false,
          requiresSubscription: false
        },
        {
          id: 'community_theme_2',
          name: 'Electronic Dreams',
          displayName: 'Electronic Dreams',
          description: 'Futuristic electronic sounds with synth waves',
          createdBy: 'synth_master_99',
          ...
          version: '1.2',
          category: 'electronic',
          tags: ['electronic', 'synth', 'futuristic', 'energy'],
          rating: 4.3,
          downloads: 892,
          popularity: 0.78,
          sounds: {
            ...
            alarms: {
              gentle: { type: 'built-in', soundId: 'alarm.energetic_beep' },
              energetic: { type: 'built-in', soundId: 'alarm.energetic_beep' },
              classic: { type: 'built-in', soundId: 'alarm.classic_beep' }
            }
          },
          metadata: {
  ...
  async shareThemeWithCommunity(theme: CustomSoundTheme): Promise<boolean> {
    try {
      // In a real implementation, this would upload to Supabase
      // For now, just simulate success
      console.log(`Sharing theme "${theme.name}" with community`);
      return true;
    } catch (error) {
      console.error('Error sharing theme with community:', error);
      return false;
    }
  }

  async rateTheme(themeId: string, userId: string, rating: number): Promise<boolean> {
    try {
      ...
      console.log(`User ${userId} rated theme ${themeId} with ${rating} stars`);
      return true;
    } catch (error) {
  ...
  async incrementThemeDownloads(themeId: string): Promise<boolean> {
    try {
      // In a real implementation, this would update the download count in Supabase
      console.log(`Incremented download count for theme ${themeId}`);
      return true;
    } catch (error) {
      console.error('Error incrementing theme downloads:', error);
      return false;
    }
  }

  async getThemeComments(themeId: string): Promise<ThemeComment[]> {
    try {
      ...
      return [
        {
          ...
          comment: 'Love this theme! Great for relaxation.',
          rating: 5,
        ...
        },
        {
          ...
          userId: 'user_2',
          userName: 'Sarah Chen',
          comment: 'Really nice ambient sounds, but could use more variety.',
          rating: 4,
  ...
  async addThemeComment(themeId: string, userId: string, userName: string, comment: string, rating?: number): Promise<boolean> {
    try {
      // In a real implementation, this would save to Supabase
      console.log(`User ${userName} commented on theme ${themeId}: ${comment}`);
      return true;
    } catch (error) {
      console.error('Error adding theme comment:', error);
      return false;
    }
  }
}

// Theme Comment Interface
