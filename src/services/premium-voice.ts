...
export class PremiumVoiceService {
  ...
  static async generateAlarmSpeech(
    ...
    if (isPremiumPersonality) {
      const hasPremiumPersonalities =
        await SubscriptionService.hasFeatureAccess(
          userId,
          "premiumPersonalities",
        );
      if (!hasPremiumPersonalities) {
        ...
        return await this.generateAlarmSpeech(
          fallbackAlarm,
          userId,
          customMessage,
        );
    ...
    try {
      const enhancedMessage =
        customMessage || this.generatePersonalityMessage(alarm);
   ...
   */
  private static enhanceNuclearMessage(
    message: string,
    challengeType: string,
  ): string {
    ...
    const prefix =
      nuclearPrefixes[Math.floor(Math.random() * nuclearPrefixes.length)];
    const suffix =
      nuclearSuffixes[Math.floor(Math.random() * nuclearSuffixes.length)];
