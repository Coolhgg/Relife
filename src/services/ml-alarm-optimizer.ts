...
export class MLAlarmOptimizer {
    ...
    try {
      ...
      if (events && events.length > 0) {
        ...
        const eventTime = new Date(
          `${targetDate.toDateString()} ${firstEvent.time}`,
        );
        ...
        const timeDiff =
          (eventTime.getTime() - alarmTime.getTime()) / (1000 * 60); // minutes
  ...
  private static async analyzeLocationFactor(
    ...
    try {
      ...
      const patterns = this.behaviorData.get(userId) || [];
      const locationPattern = patterns.find(
        (p) => p.patternType === "location",
      );
  ...
  private static async analyzeHealthFactor(
    ...
    const patterns = this.behaviorData.get(userId) || [];
    const healthPattern = patterns.find(
      (p) => p.patternType === "sleep_quality",
    );
  ...
  private static calculateOptimalAdjustment(
    factors: PredictionFactor[],
  ): number {
    let weightedSum = 0;
    let totalWeight = 0;
