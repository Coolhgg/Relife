...
export class NuclearModeService {
  ...
  private challengeTemplates: NuclearModeChallenge[] = [
    ...
    // Physical movement challenge
    {
      ...
      successCriteria:
        "Complete all physical exercises as detected by phone sensors",
    ...
    // QR Code hunt
    {
      ...
      successCriteria:
        "Find all QR codes in correct sequence and enter final code",
   ...
   */
  async startNuclearSession(
    alarm: Alarm,
    user: User,
  ): Promise<NuclearModeSession> {
   ...
   */
  async getChallengesForAlarm(
    alarmId: string,
  ): Promise<NuclearModeChallenge[]> {
    try {
   ...
   */
  private calculateSessionDifficulty(
    challenges: NuclearModeChallenge[],
  ): number {
  ...
  private async calculatePerformance(
    ...
    const successfulAttempts = session.challenges.filter(
      (c) => c.successful,
    ).length;
  ...
  private calculatePersistenceScore(session: NuclearModeSession): number {
    ...
    return Math.max(
      0,
      ((maxAttempts - avgAttemptsPerChallenge) / maxAttempts) * 100,
    );
  }
