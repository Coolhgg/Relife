...
export class AlarmBattleIntegrationService {
  ...
  static getInstance(): AlarmBattleIntegrationService {
    if (!AlarmBattleIntegrationService.instance) {
      AlarmBattleIntegrationService.instance =
        new AlarmBattleIntegrationService();
    }
  ...
  async handleAlarmTrigger(
    ...
    try {
      ...
      if (battle.status !== "active") {
        console.info(
          `Battle ${battle.id} is not active (status: ${battle.status})`,
        );
        return;
      }

      // Find user's participation in the battle
      const participation = battle.participants.find(
        (p) => p.userId === user.id,
      );
      if (!participation) {
        console.warn(
          `User ${user.id} is not a participant in battle ${battle.id}`,
        );
        return;
      }
    ...
    try {
      ...
      AppAnalyticsService.getInstance().track("alarm_battle_dismissal", {
        ...
        delayMinutes: Math.round(
          (dismissalTime.getTime() -
            new Date(alarmInstance.scheduledTime).getTime()) /
            60000,
        ),
  ...
  async handleAlarmSnooze(
    ...
    try {
      ...
      if (!battle.settings.allowSnooze) {
        AppAnalyticsService.getInstance().track(
          "alarm_battle_snooze_violation",
          {
            alarmId: alarmInstance.alarmId,
            battleId: battle.id,
            userId: user.id,
      ...
      if (snoozeCount > battle.settings.maxSnoozes) {
        AppAnalyticsService.getInstance().track(
          "alarm_battle_snooze_limit_exceeded",
          {
            alarmId: alarmInstance.alarmId,
            battleId: battle.id,
            userId: user.id,
...
export const alarmBattleIntegration =
  AlarmBattleIntegrationService.getInstance();
