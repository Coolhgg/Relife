...
class DIValidator {
  ...
  private async validateServiceHealth(): Promise<void> {
    await this.runValidationStage('Service Health', async () => {
      ...
      if (!healthResult.healthy && Object.values(healthResult.services).some(s => s.critical && s.status === 'unhealthy')) {
  ...
  private async validatePerformance(): Promise<void> {
    await this.runValidationStage('Performance Validation', async () => {
      ...
      return {
        ...
        performanceGrade: initTime < 2000 && healthTime < 500 ? 'A' : initTime < 5000 && healthTime < 1000 ? 'B' : 'C',
      };
    });
  }
