...
export class OfflineSleepTracker {
  ...
  private async loadFromStorage(): Promise<void> {
    try {
      ...
      const goals = SecurityService.secureStorageGet(
        this.STORAGE_KEYS.SLEEP_GOALS,
      );
      ...
      console.error(
        "[OfflineSleepTracker] Failed to load from storage:",
        error,
      );
    }
  }
  ...
  async recordInterruption(
    ...
    console.log(
      "[OfflineSleepTracker] Recorded sleep interruption:",
      type,
      severity,
    );
  }
  ...
  private async updateGoalsProgress(session: SleepSession): Promise<void> {
    try {
        ...
        switch (goal.type) {
          ...
          case "bedtime_consistency":
            ...
            if (recentSessions.length > 0) {
              ...
              const bedtimeHour =
                bedtime.getHours() + bedtime.getMinutes() / 60;
      ...
      console.error(
        "[OfflineSleepTracker] Failed to update goals progress:",
        error,
      );
    }
  }
  ...
  private async updateSleepAnalytics(): Promise<void> {
    try {
      ...
      this.sleepAnalytics = {
        weeklyAverage:
          weeklyDurations.reduce((sum, dur) => sum + dur, 0) /
          weeklyDurations.length,
        weeklyQuality:
          weeklyQualities.reduce((sum, qual) => sum + qual, 0) /
          weeklyQualities.length,
      ...
      console.error(
        "[OfflineSleepTracker] Failed to update sleep analytics:",
        error,
      );
    }
  }
  ...
  private async generateInsights(): Promise<void> {
    try {
      ...
      const bedtimeConsistency =
        this.calculateBedtimeConsistency(recentSessions);
      ...
      if (
        this.sleepAnalytics &&
        this.sleepAnalytics.improvementTrend === "improving"
      ) {
        insights.push({
      ...
      const existingInsightTypes = this.sleepInsights.map(
        (i) => i.type + "_" + i.title,
      );
      ...
      console.error(
        "[OfflineSleepTracker] Failed to generate insights:",
        error,
      );
    }
  }
  ...
  private handleOffline(): void {
    this.isOnline = false;
    console.log(
      "[OfflineSleepTracker] Going offline, continuing offline tracking...",
    );
  }

  private handleSyncComplete(data: any): void {
    console.log(
      "[OfflineSleepTracker] Sync completed via service worker:",
      data,
    );
  }
  ...
  private async saveCurrentSession(): Promise<void> {
    try {
      ...
      console.error(
        "[OfflineSleepTracker] Failed to save current session:",
        error,
      );
    }
  }
