...
export class VoiceService {
    ...
    if (userId) {
      try {
        const premiumMessage =
          await PremiumVoiceService.generatePremiumAlarmMessage(alarm, userId);
        if (premiumMessage) {
          return premiumMessage;
        }
      } catch (error) {
        console.warn(
          "Premium voice service failed, falling back to basic:",
          error,
        );
      }
    }
  ...
  static async preloadAlarmMessages(
    alarms: Alarm[],
    userId?: string,
  ): Promise<void> {
    // Preload voice messages for all alarms
    const promises = alarms.map((alarm) =>
      this.generateAlarmMessage(alarm, userId),
    );
  ...
  static async testVoice(mood: VoiceMood, userId?: string): Promise<void> {
    ...
    if (userId) {
      try {
        ...
        console.warn(
          "Premium voice test failed, falling back to basic:",
          error,
        );
      }
    }
