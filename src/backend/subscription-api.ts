...
export class SubscriptionAPIHandler {
  ...
  async handleSubscriptionRequest(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (removePaymentMethodMatch && method === "DELETE") {
        return await this.removePaymentMethod(
          removePaymentMethodMatch[1],
          origin,
        );
      }
      ...
      // Discount Validation
      if (
        url.pathname === "/api/stripe/coupons/validate" &&
        method === "POST"
      ) {
      ...
      if (
        url.pathname === "/api/subscription/feature-access" &&
        method === "POST"
      ) {
      ...
      // Usage Tracking
      if (
        url.pathname === "/api/subscription/track-usage" &&
        method === "POST"
      ) {
  ...
  private async updateSubscription(
    ...
    try {
      ...
      if (planId) {
        ...
        if (!priceId) {
          return errorResponse(
            "Invalid billing interval for plan",
            400,
            origin,
          );
        }
  ...
  private async cancelSubscription(
    ...
    const { reason, feedback, cancelImmediately, surveyData } =
      await request.json();
  ...
  private async validateDiscountCode(
    ...
    try {
      ...
      try {
        ...
        return successResponse(
          { valid: coupon.valid, discount: coupon },
          origin,
        );
      } catch {
        return successResponse({ valid: false }, origin);
