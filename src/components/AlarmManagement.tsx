...
export function AlarmManagement({
  ...
  const [selectedAlarmForSmart, setSelectedAlarmForSmart] =
  ...
  const getDaysSummary = (days: number[]) => {
    ...
    if (days.length === 5 && !days.includes(6) && !days.includes(0))
      return "Weekdays";
    if (days.length === 2 && days.includes(6) && days.includes(0))
      return "Weekends";
    return days
      .map((dayNum) => DAYS.find((d) => d.number === dayNum)?.short)
      .join(", ");
  };
              ...
              {editingAlarm === alarm.id ? (
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {DAYS.map((day) => (
                        <Badge
                          key={day.value}
                          variant={
                            formData.days.includes(day.number)
                              ? "default"
                              : "secondary"
                          }
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      {
                        DIFFICULTIES.find((d) => d.value === alarm.difficulty)
                          ?.emoji
                      }
                      {
                        DIFFICULTIES.find((d) => d.value === alarm.difficulty)
