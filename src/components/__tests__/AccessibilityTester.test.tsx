...
describe("AccessibilityTester", () => {
  ...
  describe("Rendering", () => {
    it("renders when visible", () => {
      ...
      expect(
        screen.getByText("Test accessibility features"),
      ).toBeInTheDocument();
    });

    it("does not render when not visible", () => {
      renderWithProviders(
        <AccessibilityTester {...defaultProps} isVisible={false} />,
      );

      expect(
        screen.queryByText("Accessibility Tester"),
  ...
  describe("ARIA Compliance Testing", () => {
    it("validates ARIA labels and roles", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/aria compliance results/i),
        ).toBeInTheDocument();
      });
    });
