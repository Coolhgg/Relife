...
describe("AlarmForm", () => {
  ...
  describe("rendering", () => {
    test("renders new alarm form by default", () => {
      ...
      expect(
        screen.getByRole("button", { name: /create alarm/i }),
      ).toBeInTheDocument();
      expect(
    ...
    test("renders edit alarm form when alarm prop is provided", () => {
      ...
      expect(
        screen.getByRole("button", { name: /update alarm/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("form interactions", () => {
    ...
    test("changes voice mood selection", async () => {
      ...
      const drillSergeantButton = screen
        .getByText("Drill Sergeant")
        .closest("button");
  ...
  describe("form validation", () => {
    test("shows validation errors for invalid time", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/hours must be between 0 and 23/i),
        ).toBeInTheDocument();
      });
    ...
    test("announces validation errors to screen readers", async () => {
      ...
      await waitFor(() => {
        const errorMessage = screen.getByText(
          /hours must be between 0 and 23/i,
        );
