...
describe("AlarmRinging", () => {
  ...
  describe("voice recognition", () => {
    ...
    test("shows listening indicator when voice recognition is active", async () => {
      ...
      expect(screen.getByTestId("voice-indicator")).toHaveClass(
        "animate-pulse",
      );
    });
    ...
    test("handles voice commands correctly", async () => {
      ...
      const resultHandler =
        mockSpeechRecognition.addEventListener.mock.calls.find(
          (call) => call[0] === "result",
        )?.[1];
  ...
  describe("interactions", () => {
    ...
    test("resets double-tap after timeout", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.queryByText(/tap again to dismiss/i),
  ...
  describe("accessibility", () => {
    ...
    test("announces voice recognition status", async () => {
      ...
      expect(
        screen.getByLabelText("Voice recognition active"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("error handling", () => {
    test("handles voice recognition errors gracefully", async () => {
      ...
      if (errorHandler) {
        ...
        expect(
          screen.getByText(/voice recognition unavailable/i),
        ).toBeInTheDocument();
      }
    });
    ...
    test("handles missing Web Speech API", () => {
      ...
      expect(
        screen.getByText(/voice commands not supported/i),
      ).toBeInTheDocument();
    });
