...
const MobileTester: React.FC<MobileTesterProps> = ({ isVisible, onClose }) => {
  ...
  const updateTestResult = (
    ...
    setTestResults((prev: any) => // auto: implicit any {
      const existing = prev.find((r: any) => // auto: implicit any r.name === name);
      if (existing) {
                ...
                <div className="grid grid-cols-4 gap-4 text-center text-sm">
                  <div>
                    <div className="text-green-600 font-medium">
                      {testResults.filter((r: any) => // auto: implicit any r.status === 'pass').length}
                    </div>
                    <div className="text-gray-600">Passed</div>
                  </div>
                  <div>
                    <div className="text-red-600 font-medium">
                      {testResults.filter((r: any) => // auto: implicit any r.status === 'fail').length}
                    </div>
                    <div className="text-gray-600">Failed</div>
                  </div>
                  <div>
                    <div className="text-yellow-600 font-medium">
                      {testResults.filter((r: any) => // auto: implicit any r.status === 'warning').length}
                    </div>
                    <div className="text-gray-600">Warnings</div>
                  </div>
                  <div>
                    <div className="text-blue-600 font-medium">
                      {testResults.filter((r: any) => // auto: implicit any r.status === 'info').length}
                    </div>
