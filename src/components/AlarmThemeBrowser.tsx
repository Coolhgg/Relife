      ...
      <div className="browser-header p-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <select
              value={sortMode}
              onChange={(e: any) => // auto: implicit any setSortMode(e.target.value as SortMode)}
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e: any) => // auto: implicit any setShowPremiumOnly(e.target.checked)}
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e: any) => // auto: implicit any setShowFavoritesOnly(e.target.checked)}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      Favorites Only
        ...
        {contextualRecommendations.length > 0 && (
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {contextualRecommendations.map((combo: any) => // auto: implicit any (
                <ThemeCard
                  key={combo.id}
                  combination={combo}
        ...
        <AnimatePresence mode="wait">
          {viewMode === 'collections' && (
            <motion.div
              ...
              className="space-y-8"
            >
              {collections.map((collection: any) => // auto: implicit any (
                <CollectionView
                  key={collection.id}
          ...
          {viewMode === 'grid' && (
            ...
            >
              {filteredCombinations.map((combo: any) => // auto: implicit any (
                <ThemeCard
                  key={combo.id}
                  combination={combo}
                  isSelected={selectedTheme === combo.id}
...
const CollectionView: React.FC<CollectionViewProps> = ({
  ...
  <div className="collection-view">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {combinations.map((combo: any) => // auto: implicit any (
        <ThemeCard
          key={combo.id}
          combination={combo}
          isSelected={selectedTheme === combo.id}
