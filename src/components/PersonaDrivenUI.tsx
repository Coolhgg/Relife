...
import { User, Clock, Star, Users, GraduationCap, Crown, Target, TrendingUp } from 'lucide-react';

interface PersonaUIProps {
  userId: string;
  currentTier: 'free' | 'basic' | 'premium' | 'pro' | 'student' | 'lifetime';
  userPersona?: 'struggling_sam' | 'busy_ben' | 'professional_paula' | 'enterprise_emma' | 'student_sarah' | 'lifetime_larry';
...
export function PersonaFeatureHighlights({ userPersona }: { userPersona: string }) {
  const getPersonaFeatures = (persona: string) => {
    const features = {
      struggling_sam: [
        { icon: "🆓", title: "Always Free", description: "Core features never cost a thing" },
        { icon: "👥", title: "Join 50k+ Users", description: "Trusted by thousands daily" },
        { icon: "⚡", title: "Instant Setup", description: "Working in under 30 seconds" }
      ],
      busy_ben: [
        { icon: "☕", title: "Coffee Price", description: "Less than $4/month for better mornings" },
        { icon: "🎵", title: "Custom Sounds", description: "Upload your perfect wake-up sound" },
        { icon: "⏰", title: "Multiple Alarms", description: "Work, weekend, and everything between" }
      ],
      professional_paula: [
        { icon: "📈", title: "Productivity Boost", description: "AI-optimized wake times for peak performance" },
        { icon: "📅", title: "Calendar Sync", description: "Automatic adjustment based on your schedule" },
        { icon: "📊", title: "Analytics", description: "Track patterns and optimize over time" }
      ],
      enterprise_emma: [
        { icon: "👥", title: "Team Features", description: "Manage and motivate entire teams" },
        { icon: "🔌", title: "API Access", description: "Integrate with your existing tools" },
        { icon: "🏷️", title: "White Label", description: "Customize branding for your organization" }
      ],
      student_sarah: [
        { icon: "🎓", title: "Student Discount", description: "50% off because education matters" },
        { icon: "📚", title: "Academic Calendar", description: "Sync with semester and exam schedules" },
        { icon: "👫", title: "Study Groups", description: "Coordinate wake-up times with friends" }
      ],
      lifetime_larry: [
        { icon: "👑", title: "Own It Forever", description: "One payment, lifetime access" },
        { icon: "🌟", title: "Founder Status", description: "Exclusive badge and early features" },
        { icon: "💎", title: "Premium Support", description: "Priority help whenever you need it" }
      ]
    };
...
export function PersonaDrivenUI({ userId, currentTier, userPersona, onPersonaDetected }: PersonaUIProps) {
