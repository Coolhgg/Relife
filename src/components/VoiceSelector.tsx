...
const VoiceSelector: React.FC<VoiceSelectorProps> = ({
  ...
  const [filter, setFilter] = useState<"all" | "free" | "premium" | "ultimate">(
    "all",
  );
  const [showPersonalityDetails, setShowPersonalityDetails] = useState<
    string | null
  >(null);
  ...
  const handleVoiceSelect = async (voiceMood: VoiceMood) => {
    const canAccess = await PremiumVoiceService.canAccessVoice(
      user.id,
      voiceMood,
    );
    if (canAccess) {
    ...
    <div className="bg-white rounded-xl p-6">
      ...
      <div className="space-y-8">
        {Object.entries(voicesByCategory).map(([category, voices]) => (
          <div key={category}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {voices.map((voice) => {
                const personality = PremiumVoiceService.getVoicePersonality(
                  voice.id,
                );
                const isLocked =
                  personality &&
                  !availableVoices.some((av) => av.id === voice.id);
      ...
      {user.subscriptionTier === "free" && showUpgradePrompts && (
        <div className="mt-8 bg-gradient-to-r from-orange-500 to-red-500 text-white p-6 rounded-xl">
          ...
          <p className="mb-4 text-orange-100">
            Get access to {availableVoices.length - 6}+ premium voice
            personalities, including celebrity chefs, zen masters, robot
            companions, and more! Plus voice cloning with Ultimate tier.
          </p>
          <button
      ...
      {Object.keys(voicesByCategory).length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            Try adjusting your filter to see more voices.
          </p>
        </div>
      )}
    </div>
