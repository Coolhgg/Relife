...
const PremiumFeaturePreview: React.FC<PremiumFeaturePreviewProps> = ({
  ...
  useEffect(() => {
    if (feature === 'nuclearMode' && isPlaying) {
      const interval = setInterval(() => {
        setNuclearIntensity((prev: any) => // auto: implicit any (prev + 1) % 100);
      }, 50);
  ...
  const renderNuclearModePreview = () => (
    ...
    >
      ...
      <div
        className="absolute inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity cursor-pointer"
        onClick={onUpgrade}
      >
      ...
      <div className="relative z-10">
        ...
        <div className="p-4 bg-black bg-opacity-30 rounded-xl mb-4">
          ...
          <p className="font-medium italic">
            "{premiumPersonalities[currentDemo]?.message}"
          </p>
        </div>
