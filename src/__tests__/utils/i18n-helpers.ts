  ...
  mockReactI18next(defaultLocale: string = "en"): void {
    const mockI18n = {
      ...
      t: (key: string, options: any = {}) => {
        const keys = key.split(".");
        let value: any =
          mockTranslations[mockI18n.language] ||
          mockTranslations[defaultLocale];
    ...
    global.Intl.NumberFormat = jest.fn().mockImplementation(() => ({
      format: jest.fn((number: number) => {
        ...
        const decimalPart = parts[1]
          ? config.numberFormat.decimal + parts[1]
          : "";
...
export const _alarmI18nUtils = {
  ...
  testAlarmNotificationMessages(locale: string): void {
    ...
    const alarmDeletedMessage = mockContext.t(
      "alarm.notifications.alarmDeleted",
    );
    expect(alarmDeletedMessage).toBeDefined();
    expect(alarmDeletedMessage.length).toBeGreaterThan(0);

    // Test alarm ringing notification
    const alarmRingingMessage = mockContext.t(
      "alarm.notifications.alarmRinging",
    );
