...
describe("Core Factories Type Safety", () => {
  describe("createTestUser Type Safety", () => {
    it("should return properly typed User object without any casts", () => {
      ...
      expect(
        Array.isArray(personalization.colorPreferences.favoriteColors),
      ).toBe(true);
      expect(typeof personalization.colorPreferences.colorblindFriendly).toBe(
        "boolean",
      );
      expect(
        typeof personalization.typographyPreferences.preferredFontSize,
      ).toBe("string");
      expect(["small", "medium", "large", "extra-large"]).toContain(
        personalization.typographyPreferences.preferredFontSize,
      );
      expect(typeof personalization.motionPreferences.enableAnimations).toBe(
        "boolean",
      );
      expect(typeof personalization.soundPreferences.enableSounds).toBe(
        "boolean",
      );
