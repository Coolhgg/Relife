...
export const createTestUser = (options: CreateUserOptions = {}): User => {
  ...
  return {
    ...
    createdAt: joinDate,
    subscription:
      tier !== "free" ? ({ id: generateId("sub") } as any) : undefined,
    stripeCustomerId:
...
const createTestBattleSettings = (options: {
  ...
  return {
    ...
    requireVerification:
      premium && faker.datatype.boolean({ probability: 0.4 }),
    trashTalkEnabled: faker.datatype.boolean({ probability: 0.3 }),
    penalties: {
...
const createTestBattlePrize = (options: { premium: boolean }) => {
  ...
  return {
    ...
    badges: randomSubset(
      ["early-bird", "consistent", "warrior", "champion"],
      1,
      2,
    ),
    premiumDays: premium ? faker.number.int({ min: 1, max: 30 }) : 0,
    customization: premium
      ? randomSubset(["theme", "voice", "sound"], 0, 2)
      : [],
  } as any;
};
