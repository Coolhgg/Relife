...
export const createTestSubscription = (options: CreateSubscriptionOptions = {}): Subscription => {
  const {
    ...
    billingInterval = faker.helpers.arrayElement(['month', 'year', 'lifetime'] as BillingInterval[]),
  ...
  return {
    id: subscriptionId,
    userId,
    stripeSubscriptionId: tier !== 'free' ? `sub_${faker.string.alphanumeric(14)}` : undefined,
    ...
    cancelAtPeriodEnd: status === 'canceled' || faker.datatype.boolean({ probability: 0.1 }),
    canceledAt: status === 'canceled' ? faker.date.recent({ days: 30 }) : undefined,
    endedAt: ['ended', 'incomplete_expired'].includes(status) ? faker.date.recent({ days: 30 }) : undefined,
    createdAt: faker.date.recent({ days: 365 }),
    updatedAt: faker.date.recent({ days: 30 }),
    metadata: {
      source: faker.helpers.arrayElement(['web', 'mobile', 'referral']),
      campaign: faker.datatype.boolean({ probability: 0.3 }) ? faker.string.alphanumeric(8) : undefined,
      referrer: faker.datatype.boolean({ probability: 0.2 }) ? generateId('user') : undefined
    }
  };
};
...
export const createTestVoice = (options: CreateVoiceOptions = {}): PremiumVoice => {
  ...
  return {
    ...
    mood,
    tier,
    category: faker.helpers.arrayElement(['character', 'celebrity', 'professional', 'community']) as PremiumVoiceCategory,
    language,
    accent: faker.helpers.arrayElement(['american', 'british', 'australian', 'neutral']),
    ...
    samples: Array.from({ length: faker.number.int({ min: 3, max: 8 }) }, () => createTestVoiceSample()),
    isCustom,
    createdBy: isCustom ? generateId('user') : undefined,
    tags: randomSubset(['energetic', 'calm', 'funny', 'serious', 'friendly', 'intense', 'soothing', 'powerful'], 2, 4),
...
const createTestVoiceSample = (): VoiceSample => ({
  ...
  context: faker.helpers.arrayElement(['wake-up', 'motivation', 'reminder', 'celebration']),
...
export const createTestCustomSound = (options: CreateCustomSoundOptions = {}): CustomSound => {
  ...
  return {
    ...
    category,
    tags: randomSubset(['relaxing', 'energizing', 'focus', 'sleep', 'meditation', 'workout', 'study'], 1, 3),
    isCustom,
...
export const createTestAnalytics = (options: CreateAnalyticsOptions = {}): PremiumAnalytics => {
  ...
  return {
    ...
    recommendations: Array.from({ length: faker.number.int({ min: 3, max: 8 }) }, () => createTestAnalyticsRecommendation()),
    trends: Array.from({ length: faker.number.int({ min: 5, max: 15 }) }, () => createTestAnalyticsTrend()),
    comparisons: createTestAnalyticsComparison(),
    goals: Array.from({ length: faker.number.int({ min: 2, max: 6 }) }, () => createTestAnalyticsGoal()),
    achievements: Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, () => createTestAnalyticsAchievement()),
