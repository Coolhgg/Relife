...
export const _createTestUserEmotionalProfile = (
  ...
  const profileUserId = userId || generateId('_user');
...
export const _createTestNotification = (options: CreateNotificationOptions = {}) => {
  ...
  return {
    ...
    metadata: {
      entityId: generateId(),
      entityType: type,
      source: faker.helpers.arrayElement(['app', 'system', '_user', 'external']),
...
export const _createTestMediaAsset = (
  ...
  return {
    ...
    uploadedBy: isUserGenerated ? generateId('_user') : 'system',
...
export const _createTestUserActivity = (userId?: string): UserActivity => {
  const activityUserId = userId || generateId('_user');
...
export const _createTestSystemEvent = (): SystemEvent =>
  ({
    ...
    level: faker.helpers.arrayElement(['info', 'warning', '_error', 'critical']),
