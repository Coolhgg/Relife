...
export const createTestAchievement = (options: CreateAchievementOptions = {}): Achievement => {
  ...
  const achievementNames = {
    alarm: [
      'Early Bird', 'Rise and Shine', 'Morning Warrior', 'Dawn Breaker',
      'Sunrise Champion', 'First Light', 'Morning Glory', 'Alarm Master'
    ],
    battle: [
      'Battle Veteran', 'Victory Seeker', 'Combat Ready', 'Champion Fighter',
      'Arena Master', 'Tournament Winner', 'Battle Royale', 'Gladiator'
    ],
    social: [
      'Team Player', 'Friend Maker', 'Social Butterfly', 'Community Leader',
      'Mentor', 'Helper', 'Connector', 'Ambassador'
    ],
    consistency: [
      'Streak Master', 'Consistent Performer', 'Habit Builder', 'Dedication King',
    ...
    special: [
      ...
      'Secret Finder', 'Easter Egg Hunter', 'Rare Collector', 'Legendary'
    ]
  };
  ...
  return {
    ...
    rewards: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, () => createTestAchievementReward(rarity)),
    requirements: Array.from({ length: faker.number.int({ min: 1, max: 4 }) }, () => createTestAchievementRequirement(category))
  };
};
...
export const createTestTournament = (options: CreateTournamentOptions = {}): Tournament => {
  const {
  ...
  return {
    ...
    seasonId: faker.datatype.boolean({ probability: 0.7 }) ? generateId('season') : undefined,
...
const createTestTournamentParticipant = (): TournamentParticipant => ({
  ...
  eliminatedRound: faker.datatype.boolean({ probability: 0.6 }) ? faker.number.int({ min: 1, max: 5 }) : undefined,
...
const createTestTournamentRounds = (
  ...
  if (type === 'single-elimination') {
    ...
    for (let i = 1; i <= roundCount; i++) {
      rounds.push({
        number: i,
        name: i === roundCount ? 'Final' : i === roundCount - 1 ? 'Semi-Final' : `Round ${i}`,
...
const createTestTournamentMatch = () => ({
  ...
  completedAt: faker.datatype.boolean({ probability: 0.7 }) ? generateTimestamp({ past: 7 }) : undefined
});
...
export const createTestTeam = (options: CreateTeamOptions = {}): Team => {
  const {
  ...
  return {
    id: teamId,
    name: faker.helpers.arrayElement([
      'Morning Warriors', 'Early Birds United', 'Dawn Breakers', 'Sunrise Squad',
      'Wake Up Champions', 'Rise & Grind Crew', 'Morning Glory Gang', 'First Light Team'
    ]),
    description: faker.lorem.sentence(),
    members,
    createdBy,
    ...
    achievements: Array.from({ length: faker.number.int({ min: 0, max: 10 }) }, () => generateId('achievement')),
    currentSeason: faker.datatype.boolean({ probability: 0.8 }) ? generateId('season') : undefined,
...
export const createTestSeason = (options: CreateSeasonOptions = {}): Season => {
  ...
  return {
    id: seasonId,
    number: seasonNumber,
    name: theme || faker.helpers.arrayElement([
      'Spring Awakening', 'Summer Sunrise', 'Autumn Challenge', 'Winter Warriors',
      'New Year Revolution', 'Back to School', 'Holiday Spirit', 'Championship Season'
    ]),
    description: faker.lorem.sentence(),
    status,
    startDate,
    endDate,
    theme: theme || faker.helpers.arrayElement(['nature', 'holiday', 'challenge', 'community']),
    rewards: {
    ...
    tournaments: Array.from({ length: faker.number.int({ min: 2, max: 8 }) }, () => generateId('tournament')),
