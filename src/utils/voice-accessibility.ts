...
export class VoiceAccessibilityService {
  ...
  private setupSpeechRecognition(): void {
    ...
    this.recognition.onstart = () => {
      ...
      this.provideFeedback(
        "Voice recognition started. Listening for commands.",
      );
    };
  ...
  private handleConfirmation(transcript: string): void {
    const isConfirmed = ["yes", "confirm", "okay", "proceed", "do it"].some(
      (word) => transcript.includes(word),
    );

    const isRejected = ["no", "cancel", "stop", "abort", "nevermind"].some(
      (word) => transcript.includes(word),
    );
  ...
  private listAlarms(): void {
    ...
    alarms.forEach((alarm, index) => {
      ...
      const status =
        statusElement?.getAttribute("data-active") === "true"
          ? "active"
          : "inactive";
  ...
  private adjustVoiceSpeed(delta: number): void {
    this.state.voiceSpeed = Math.max(
      0.5,
      Math.min(2.0, this.state.voiceSpeed + delta),
    );
    this.provideFeedback(
      `Voice speed set to ${this.state.voiceSpeed.toFixed(1)}x`,
    );
  }
  ...
  private readVoiceCommands(): void {
    const commands = Array.from(this.commands.values())
      .filter(
        (cmd) =>
          cmd.enabled && this.state.enabledCategories.includes(cmd.category),
      )
