...
async function storeOfflineEvent(category, event) {
  /* Implementation */
}
async function logAlarmEvent(alarm, eventType, metadata = {}) {
  /* Implementation */
}
...
async function queueEmotionalEvent(event) {
  emotionalQueue.push(event);
}
...
// Enhanced service worker status with cache info
function getServiceWorkerStatus() {
  return {
    version: APP_VERSION,
    isOnline: isOnline,
    lastSync: lastSyncTime ? new Date(lastSyncTime).toISOString() : null,
    alarmCount: alarmTimeouts.size,
    analyticsQueued: analyticsQueue.length,
    emotionalQueued: emotionalQueue.length,
    cacheStats: {
      hits: cacheStats.hits,
      misses: cacheStats.misses,
      hitRatio: cacheStats.hitRatio.toFixed(2),
      lastCleanup: new Date(cacheStats.lastCleanup).toISOString(),
    },
    features: {
      smartCaching: true,
      backgroundSync: true,
      pushNotifications: !!pushSubscription,
