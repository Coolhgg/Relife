...
const CACHE_STRATEGIES = {
  ...
  NETWORK_FIRST: [/\/api\/auth/, /\/api\/users/, /\/api\/sync/],
...
self.addEventListener("notificationclick", (event) => {
  console.log(
    "Enhanced SW: Notification clicked:",
    event.action,
  ...
  try {
    switch (type) {
      ...
      case "GET_SCHEDULED_ALARMS":
        const scheduledData = Array.from(scheduledAlarms.entries()).map(
          ([id, info]) => ({
            id,
            alarm: info.alarm,
    ...
    if (nextTime) {
      ...
      if (msUntilAlarm > 0) {
        ...
        const timeoutId = setTimeout(
          ...
          Math.min(msUntilAlarm, 2147483647),
        ); // Max timeout value
        ...
        console.log(
          `Enhanced SW: Alarm ${alarm.id} scheduled for`,
        ...
        console.log(
          `Enhanced SW: Alarm ${alarm.id} scheduled time has passed, triggering immediately`,
        );
        await triggerEnhancedAlarm(alarm);
      }
    } else {
      console.log(
        `Enhanced SW: No next trigger time found for alarm ${alarm.id}`,
      );
      await removeScheduledAlarm(alarm.id);
    }
  } catch (error) {
    console.error(`Enhanced SW: Error scheduling alarm ${alarm.id}:`, error);
    await logEnhancedAlarmEvent(alarm, "scheduling_error", {
      error: error.message,
    });
  }
}
...
async function triggerEnhancedAlarm(alarm) {
  console.log(
    `Enhanced SW: Triggering enhanced alarm ${alarm.id} at ${new Date().toISOString()}`,
  );
  ...
  try {
    ...
    if (notificationPermission !== "granted") {
      console.warn(
        "Enhanced SW: Notifications not permitted, requesting permission...",
      );
      try {
        const permission = await Notification.requestPermission();
        notificationPermission = permission;
        ...
        console.error(
          "Enhanced SW: Error requesting notification permission:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "Enhanced SW: Error showing notification:",
        notificationError,
      );
    ...
    if (alarm.days && alarm.days.length > 0) {
      console.log(
        `Enhanced SW: Scheduling next occurrence for recurring alarm ${alarm.id}`,
      );
      ...
      setTimeout(async () => {
        try {
          ...
          console.error(
            `Enhanced SW: Error rescheduling alarm ${alarm.id}:`,
            rescheduleError,
          );
        }
      }, 1000);
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("scheduledAlarms")) {
        const alarmStore = db.createObjectStore("scheduledAlarms", {
          keyPath: "id",
        });
      ...
      if (!db.objectStoreNames.contains("alarmEvents")) {
        const eventStore = db.createObjectStore("alarmEvents", {
          keyPath: "id",
        });
      ...
      if (!db.objectStoreNames.contains("missedAlarms")) {
        const missedStore = db.createObjectStore("missedAlarms", {
          keyPath: "id",
        });
...
async function initializeAdvancedFeatures() {
  ...
  notificationPermission = (await self.registration.showNotification)
    ? "granted"
    : "denied";
...
async function processAnalyticsQueue() {
  ...
  console.log(
    `Enhanced SW: Processing ${analyticsQueue.length} queued analytics events`,
  );
...
async function updateEnhancedAlarms(alarms) {
  ...
  try {
    ...
    console.log(
      `Enhanced SW: Scheduling ${enabledAlarms.length} enabled alarms`,
    );
...
async function storeScheduledAlarm(alarm, nextTime) {
  ...
  try {
    const transaction = alarmDatabase.transaction(
      ["scheduledAlarms"],
      "readwrite",
    );
...
async function recoverScheduledAlarms() {
  ...
  try {
    ...
    request.onsuccess = async () => {
      ...
      console.log(
        `Enhanced SW: Found ${storedAlarms.length} stored alarms to recover`,
      );
      ...
      for (const storedAlarm of storedAlarms) {
        ...
        if (nextTrigger > now) {
          ...
          console.log(
            `Enhanced SW: Rescheduling alarm ${storedAlarm.id} for ${nextTrigger}`,
          );
          ...
          if (
            storedAlarm.alarmData.days &&
            storedAlarm.alarmData.days.length > 0
          ) {
            console.log(
              `Enhanced SW: Scheduling next occurrence for recurring alarm ${storedAlarm.id}`,
            );
            await scheduleEnhancedAlarm(storedAlarm.alarmData);
          } else {
            // One-time alarm that's expired, remove it
            console.log(
              `Enhanced SW: Removing expired one-time alarm ${storedAlarm.id}`,
            );
    ...
    request.onerror = () => {
      console.error(
        "Enhanced SW: Error recovering alarms from IndexedDB:",
        request.error,
      );
    };
  } catch (error) {
...
async function triggerMissedAlarm(alarm, originalTime) {
  console.log(
    `Enhanced SW: Triggering missed alarm ${alarm.id} (was scheduled for ${originalTime})`,
  );
  ...
  try {
    ...
    console.error(
      `Enhanced SW: Error triggering missed alarm ${alarm.id}:`,
      error,
    );
  }
}
...
async function performAlarmHealthCheck() {
  ...
  for (const [alarmId, alarmInfo] of scheduledAlarms) {
    ...
    if (new Date(nextTrigger) <= new Date(now - 30000)) {
      // 30 seconds grace period
      console.log(
        `Enhanced SW: Alarm ${alarmId} should have fired, triggering now`,
      );
      await triggerEnhancedAlarm(alarm);
    }
  }
    ...
    request.onsuccess = () => {
      ...
      for (const storedAlarm of storedAlarms) {
        if (!scheduledAlarms.has(storedAlarm.id)) {
          console.log(
            `Enhanced SW: Found alarm ${storedAlarm.id} in storage but not in memory, recovering...`,
          );
      ...
      for (const [alarmId] of scheduledAlarms) {
        ...
        if (!found) {
          console.log(
            `Enhanced SW: Alarm ${alarmId} in memory but not in storage, re-storing...`,
          );
          const alarmInfo = scheduledAlarms.get(alarmId);
          if (alarmInfo) {
            storeScheduledAlarm(
              alarmInfo.alarm,
              new Date(alarmInfo.nextTrigger),
            );
          }
        }
      }
      ...
      async () => {
        ...
        if (scheduledAlarms.has(alarmId)) {
          ...
          if (triggerTime > new Date()) {
            console.log(
              `Enhanced SW: Backup check - alarm ${alarmId} still pending`,
            );
            // Ensure timeout is still active
            if (!alarmTimeouts.has(alarmId)) {
              console.log(
                `Enhanced SW: Rescheduling alarm ${alarmId} from backup check`,
              );
              await scheduleEnhancedAlarm(alarmInfo.alarm);
            }
          }
...
async function syncAlarmStateWithClients() {
  const alarmState = {
    scheduledAlarms: Array.from(scheduledAlarms.entries()).map(
      ([id, info]) => ({
        id,
        nextTrigger: info.nextTrigger,
        scheduledAt: info.scheduledAt,
...
async function logEnhancedAlarmEvent(alarm, eventType, metadata = {}) {
  ...
  console.log(
    `Enhanced SW: Logging alarm event - ${eventType} for alarm ${alarm.id}`,
  );
...
async function handleEnhancedSnooze(data) {
  console.log(
    `Enhanced SW: Handling enhanced snooze for alarm ${data.alarmId}`,
  );
...
async function getAlarmStatistics() {
  ...
  try {
    const transaction = alarmDatabase.transaction(
      ["alarmEvents", "scheduledAlarms", "missedAlarms"],
      "readonly",
    );
...
async function clearMissedAlarms() {
  if (!alarmDatabase) {
    console.log(
      "Enhanced SW: Database not available for clearing missed alarms",
    );
    return;
  }
...
console.log(
  `Enhanced Service Worker v${appVersion}: Loaded and ready with comprehensive alarm reliability features`,
);
