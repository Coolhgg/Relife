...
async function handleGetRequest(request) {
  ...
  try {
    ...
    if (
      STATIC_CACHE_URLS.some(
        (cachedUrl) =>
          url.pathname === cachedUrl || url.pathname.endsWith(cachedUrl),
      )
    ) {
...
function getBackgroundSyncTag(queueKey) {
  switch (queueKey) {
    case "alarms":
      return BACKGROUND_SYNC_TAGS.ALARM_SYNC;
    case "settings":
      return BACKGROUND_SYNC_TAGS.SETTINGS_SYNC;
    default:
      return "general-sync";
  }
}
...
async function removeFromQueue(key, requestToRemove) {
  ...
  const updatedQueue = queue.filter(
    (req) =>
      req.url !== requestToRemove.url ||
      req.timestamp !== requestToRemove.timestamp,
  );
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification("Relife Alarm", options));
});
