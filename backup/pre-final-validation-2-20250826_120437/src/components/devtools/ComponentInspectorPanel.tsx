...
export const ComponentInspectorPanel: React.FC = () => {
  ...
  const renderCountMapRef = useRef<Map<string, { count: number; times: number[] }>>(
    new Map()
  );
      ...
      const findReactComponents = (node: Element, depth = 0): ComponentInfo[] => {
        ...
        if (fiberKey) {
          ...
          if (fiber && fiber.type && typeof fiber.type === 'function') {
            ...
            const renderStats = renderCountMapRef.current.get(componentName) || {
              count: 0,
              times: [],
            };
            ...
            const componentInfo: ComponentInfo = {
              ...
              averageRenderTime:
                renderStats.times.length > 0
                  ? renderStats.times.reduce((a, b) => a + b, 0) /
                    renderStats.times.length
                  : 0,
        ...
        if (depth < 10) {
          Array.from(node.children).forEach(child => {
            ...
            if (nodeComponents.length > 0) {
              nodeComponents[nodeComponents.length - 1].children.push(
                ...childComponents
              );
            } else {
      ...
      hook.onCommitFiberRoot = (...args: any[]) => {
        ...
        const updateRenderStats = (fiber: any, startTime: number) => {
          if (fiber && fiber.type && typeof fiber.type === 'function') {
            ...
            const stats = renderCountMapRef.current.get(componentName) || {
              count: 0,
              times: [],
            };
            stats.count++;
  ...
  const stats: ComponentStats = React.useMemo(() => {
    ...
    const totalRenders = Array.from(renderCountMapRef.current.values()).reduce(
      (sum, stats) => sum + stats.count,
      0
    );

    const renderTimes = Array.from(renderCountMapRef.current.entries()).filter(
      ([_, stats]) => stats.times.length > 0
    );

    const slowest = renderTimes.reduce(
      (max, [name, stats]) => {
    ...
    const fastest = renderTimes.reduce(
      (min, [name, stats]) => {
        const avgTime = stats.times.reduce((a, b) => a + b, 0) / stats.times.length;
    ...
    return components.map(component => (
      <div key={component.id} className="select-none">
        ...
        {component.isExpanded && component.children.length > 0 && (
          <div>{renderComponentTree(component.children)}</div>
        )}
      </div>
    ));
  };
      ...
      <div className="p-4 bg-gray-50 rounded-lg">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs px-2 py-1 rounded ${isTracking ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div>
                      <strong>Renders:</strong> {selectedComponent.renderCount}
                    </div>
                    <div>
                      <strong>Avg Render Time:</strong>{' '}
                      {selectedComponent.averageRenderTime.toFixed(2)}ms
                    </div>
                    <div>
                      <strong>Total Render Time:</strong>{' '}
                      {selectedComponent.totalRenderTime.toFixed(2)}ms
                    </div>
                    <div>
                      <strong>Children:</strong> {selectedComponent.children.length}
                    </div>
                  </div>
                </div>
