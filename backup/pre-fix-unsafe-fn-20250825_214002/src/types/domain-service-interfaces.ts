...
export interface IGamificationService extends BaseService {
  ...
  // Leaderboards
  getLeaderboard(type: string, timeframe?: 'daily' | 'weekly' | 'monthly' | 'all'): Promise<any[]>;
...
export interface IThemeService extends BaseService {
  ...
  // Custom Themes
  createCustomTheme(userId: string, theme: {
    name: string;
  ...
  // Theme Assets
  uploadThemeAsset(userId: string, themeId: string, assetType: string, file: Blob): Promise<string>;
...
export interface IEmotionalIntelligenceService extends BaseService {
  // Mood Analysis
  analyzeMood(userId: string, context: {
    timeOfDay: string;
    sleepQuality?: number;
    stressLevel?: number;
  ...
  // Emotional Messages
  generateEmotionalMessage(userId: string, scenario: string, mood: string): Promise<{
    title: string;
    message: string;
    tone: string;
  }>;
  ...
  // Behavior Patterns
  analyzeUserBehavior(userId: string, timeframe: number): Promise<{
    patterns: any[];
    insights: string[];
    recommendations: string[];
  }>;
...
export interface IRecommendationService extends BaseService {
  ...
  getRecommendations(userId: string, type: 'alarms' | 'themes' | 'challenges' | 'battles'): Promise<any[]>;
  
  // Smart Scheduling
  suggestOptimalAlarmTimes(userId: string, preferences: {
    wakeUpGoal: Date;
...
export interface ISleepTrackingService extends BaseService {
  // Sleep Data
  recordSleepData(userId: string, sleepData: {
    bedtime: Date;
    sleepTime: Date;
    wakeTime: Date;
  ...
  // Sleep Goals
  setSleepGoals(userId: string, goals: {
...
export interface IWellnessService extends BaseService {
  // Wellness Tracking
  recordWellnessMetric(userId: string, metric: {
  ...
  generateWellnessReport(userId: string, timeframe: 'week' | 'month' | 'quarter'): Promise<{
    summary: any;
    trends: any[];
    recommendations: string[];
    achievements: any[];
...
export interface ICalendarService extends BaseService {
  ...
  connectCalendar(userId: string, provider: 'google' | 'outlook' | 'apple', credentials: any): Promise<void>;
  ...
  // Smart Scheduling
  findOptimalMeetingTimes(userIds: string[], duration: number, preferences?: any): Promise<Date[]>;
  
  // Alarm Integration
  syncAlarmsWithCalendar(userId: string): Promise<void>;
  createAlarmFromEvent(userId: string, eventId: string, offsetMinutes: number): Promise<any>;
}
...
export interface IWeatherService extends BaseService {
  ...
  getWeatherForecast(location: { lat: number; lng: number } | string, days: number): Promise<any[]>;
  
  // Location Services
  getUserLocation(userId: string): Promise<{ lat: number; lng: number } | null>;
  setUserLocation(userId: string, location: { lat: number; lng: number }): Promise<void>;
...
export interface ICloudSyncService extends BaseService {
  ...
  // Conflict Resolution
  resolveConflict(userId: string, conflictId: string, resolution: 'local' | 'remote' | 'merge'): Promise<void>;
...
export interface IServiceEventBus extends BaseService {
  // Event Publishing
  publish<T extends ServiceEventType>(eventType: T, data: ServiceEventData[T]): Promise<void>;
  
  // Event Subscription
  subscribe<T extends ServiceEventType>(eventType: T, handler: (data: ServiceEventData[T]) => void | Promise<void>): string; // Returns subscription ID
  unsubscribe(subscriptionId: string): void;
  
  // Event History
  getEventHistory(eventType?: ServiceEventType, startDate?: Date, endDate?: Date): Promise<any[]>;
}