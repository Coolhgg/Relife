...
import { TrendingUp, TrendingDown, Brain, Target, Zap, Shield, Users, Calendar, Cloud, Activity } from 'lucide-react';
...
export const EnhancedBehavioralIntelligenceDashboard: React.FC<BehavioralIntelligenceDashboardProps> = ({
  userId,
  alarms,
  alarmEvents,
  className = '',
  ...
  useEffect(() => {
    const fetchBehavioralData = async () => {
      ...
      try {
        ...
        const recsResult = await recommendationEngine.generatePersonalizedRecommendations(
          userId,
          alarms,
          alarmEvents
        );
  ...
  const psychologicalRadarData = useMemo(() => {
    ...
    return [
      ...
      { trait: 'Conscientiousness', value: Math.round(bigFiveTraits.conscientiousness * 100) },
      ...
      { trait: 'Neuroticism', value: Math.round((1 - bigFiveTraits.neuroticism) * 100) }, // Inverted for stability
    ];
  }, [analysisData]);
  ...
  const predictiveData = useMemo(() => {
    ...
    const { sleepQualityForecast, energyLevelPrediction } = analysisData.predictiveAnalysis;
  ...
  const insightsByPriority = useMemo(() => {
    ...
    return analysisData.insights.reduce((acc: any, insight: BehavioralInsight) => {
  ...
  return (
    <div className={`p-6 space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Behavioral Intelligence Dashboard</h1>
        ...
        <div className="flex items-center space-x-4">
          <Badge variant="outline" className="flex items-center space-x-1">
            ...
            <span>AI Confidence: {Math.round((analysisData?.psychologicalProfile?.confidence || 0.7) * 100)}%</span>
          </Badge>
        </div>
      </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{analysisData?.insights?.length || 0}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{crossPlatformData ? 3 : 1}</p>
              </div>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {insightsByPriority.critical.length > 0 && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertTitle className="text-red-800">Critical Insights Detected</AlertTitle>
              <AlertDescription className="text-red-700">
                {insightsByPriority.critical.length} critical insight{insightsByPriority.critical.length !== 1 ? 's' : ''} require{insightsByPriority.critical.length === 1 ? 's' : ''} immediate attention.
                <button
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">
                        {analysisData.psychologicalProfile.stressResponse.replace('_', ' ')}
                      </Badge>
                    </div>
                    <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Detailed personality trait analysis</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {Object.entries(analysisData.psychologicalProfile.bigFiveTraits).map(([trait, value]) => (
                        <div key={trait}>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-sm text-gray-500">
                              {Math.round((trait === 'neuroticism' ? 1 - (value as number) : value as number) * 100)}%
                            </span>
                          </div>
                          <Progress 
                            value={(trait === 'neuroticism' ? 1 - (value as number) : value as number) * 100} 
                            className="h-2" 
                          />
                        </div>
                      ))}
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>What drives your behavior patterns</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {Object.entries(analysisData.psychologicalProfile.motivationalFactors).map(([factor, value]) => (
                        <div key={factor}>
                          <div className="flex items-center justify-between mb-1">
                ...
                <Card key={priority}>
                  <CardHeader>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      />
                      <span>{priority.charAt(0).toUpperCase() + priority.slice(1)} Priority Insights</span>
                      <Badge variant="secondary">{(insights as BehavioralInsight[]).length}</Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {(insights as BehavioralInsight[]).map((insight) => {
                        const IconComponent = TYPE_ICONS[insight.type as keyof typeof TYPE_ICONS] || Brain;
                        return (
                          <div 
                            key={insight.id}
                          ...
                          >
                            <div className="flex items-start space-x-3">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600 mt-1">{insight.personalizedMessage}</p>
                                <div className="flex items-center space-x-4 mt-2">
                                  ...
                                  <span className="text-xs text-gray-500">
                                    {insight.suggestedActions.length} suggested action{insight.suggestedActions.length !== 1 ? 's' : ''}
                                  </span>
                                </div>
                              </div>
                            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>AI-predicted optimal wake times for the next week</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {(analysisData?.predictiveAnalysis?.optimalWakeTimes || []).map((time: string, index: number) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {(analysisData?.predictiveAnalysis?.riskFactors || []).map((risk: any, index: number) => (
                            ...
                            <div key={index} className="flex items-start space-x-3">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                  Duration: {action.duration} • {action.required ? 'Required' : 'Optional'}
                                </p>
                              </div>
                            </div>
                          ))}
    ...
    insights: [
      {
        ...
        description: 'Your natural circadian rhythm shows strong potential for optimization',
        confidence: 0.87,
        priority: 'high',
        actionability: 'short_term',
        personalizedMessage: 'Your disciplined nature combined with consistent patterns suggests you can achieve 20% better sleep quality with targeted timing adjustments.',
        suggestedActions: [
          {
      ...
      },
      {
        ...
        description: 'High correlation detected between stress levels and alarm dismissal patterns',
        confidence: 0.92,
        priority: 'critical',
        actionability: 'immediate',
        personalizedMessage: 'Stress management could improve your wake consistency by 35%. Let\'s address this with gentle, supportive adjustments.',
        suggestedActions: [
          {
