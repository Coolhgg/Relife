  ...
  test('should have accessible alarm form', async ({ page }) => {
    ...
    if (await createButton.isVisible()) {
      ...
      if (await timeInput.isVisible()) {
        ...
        const isRequired =
          (await timeInput.getAttribute('required')) !== null ||
      ...
      const dayControls = page.locator(
        'input[type="checkbox"], [role="checkbox"], button[role="switch"]'
      );
  ...
  test('should handle delete confirmation accessibly', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      const confirmDialog = page
        .locator('[role="dialog"], .modal, .confirmation')
        .first();
      if (await confirmDialog.isVisible()) {
        // Test modal accessibility
        await a11yPatterns.testModalAccessibility(
          page,
