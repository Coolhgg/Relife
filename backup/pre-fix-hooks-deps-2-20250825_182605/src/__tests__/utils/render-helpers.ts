...
> = ({ children, _user = null, isLoading = false, isAuthenticated = false }) => {
  const mockAuthValue = {
    user,
    isLoading,
    isAuthenticated,
    signIn: jest.fn(() => Promise.resolve({ user: _user || null, _error: null })),
    signOut: jest.fn(() => Promise.resolve({ _error: null })),
    signUp: jest.fn(() => Promise.resolve({ user: _user || null, _error: null })),
    updateProfile: jest.fn(() =>
      Promise.resolve({ user: _user || null, _error: null })
    ),
    resetPassword: jest.fn(() => Promise.resolve({ _error: null })),
...
> = ({ children, theme, themes = [] }) => {
  const mockThemeValue = {
    ...
    createCustomTheme: jest.fn(() => Promise.resolve({ theme: null, _error: null })),
    deleteTheme: jest.fn(() => Promise.resolve({ _error: null })),
    syncThemes: jest.fn(() => Promise.resolve({ themes: [], _error: null })),
    isLoading: false,
...
> = ({ children, tier = 'free', isActive = true }) => {
  const mockSubscriptionValue = {
    ...
    subscribe: jest.fn((newTier: string) => {
      ...
      return Promise.resolve({ subscription: null, _error: null });
    }),
    cancel: jest.fn(() => {
...
const MockAnalyticsProvider: React.FC<MockProviderProps> = ({ children }) => {
  const mockAnalyticsValue = {
    ...
    trackError: jest.fn((_error: Error, context?: string) => {
      testConsole.debug(`Mock analytics _error: ${_error.message}`, context);
    }),
...
> = ({ children, tier = 'free', userId = 'test-_user-123' }) => {
...
> = ({ children, currentPersona = 'struggling_sam' }) => {
  ...
  return React.createElement(
    'div',
    {
      ...
      'data-current-_persona': currentPersona,
    },
    children
  );
};
...
const AllProviders: React.FC<AllProvidersProps> = ({
  ...
  if (featureAccess) {
    providerChain = React.createElement(
      ...
      { tier, userId: user?.id || 'test-_user-123' },
      providerChain
    );
  }
...
export const _renderAsUser = (
  ...
  return _renderWithProviders(ui, {
    ...
    tier: _user?.subscription?.tier || 'free',
  });
};
...
export const _renderWithPersonaAnalytics = (
  ui: ReactElement,
  _persona: string = 'struggling_sam',
  options: CustomRenderOptions = {}
) => {
  return _renderWithProviders(ui, {
    ...options,
